...
const PERS: Record<DebatePersonality, { icon: typeof TradeUpIcon; name: string; nameEn: string }> = {
...
const ACT: Record<string, { color: string; bg: string; icon: JSX.Element; label: string }> = {
  open_long: { color: 'text-green-400', bg: 'bg-green-500/20', icon: <HugeiconsIcon icon={TradeUpIcon} size={14} strokeWidth={2} />, label: 'LONG' },
  open_short: { color: 'text-red-400', bg: 'bg-red-500/20', icon: <HugeiconsIcon icon={TradeDownIcon} size={14} strokeWidth={2} />, label: 'SHORT' },
  hold: { color: 'text-blue-400', bg: 'bg-blue-500/20', icon: <HugeiconsIcon icon={MinusSignIcon} size={14} strokeWidth={2} />, label: 'HOLD' },
  wait: { color: 'text-gray-400', bg: 'bg-gray-500/20', icon: <HugeiconsIcon icon={Clock01Icon} size={14} strokeWidth={2} />, label: 'WAIT' },
  close_long: { color: 'text-blue-400', bg: 'bg-blue-500/20', icon: <HugeiconsIcon icon={Cancel01Icon} size={14} strokeWidth={2} />, label: 'CLOSE' },
  close_short: { color: 'text-blue-400', bg: 'bg-blue-500/20', icon: <HugeiconsIcon icon={Cancel01Icon} size={14} strokeWidth={2} />, label: 'CLOSE' },
}
...
function MessageCard({ msg }: { msg: DebateMessage }) {
  ...
  return (
    <div
      className="gl-onyx-panel rounded-xl p-3 overflow-hidden hover:bg-[var(--surface-secondary)]/40 transition-all debate-message-card"
    >
      ...
      >
        ...
        <span className="text-xs debate-accent font-medium tabular-nums">{msg.decision?.confidence || msg.confidence}%</span>
        <HugeiconsIcon icon={open ? ArrowUp01Icon : ArrowDown01Icon} size={14} strokeWidth={1.9} className="text-[var(--text-secondary)]" />
      </div>
          ...
          {parsed.reasoning && (
            <div className="bg-black/20 rounded-lg p-3">
              <div className="flex items-center gap-1.5 text-xs text-blue-400 font-medium mb-2">
                <HugeiconsIcon icon={BubbleChatIcon} size={13} strokeWidth={1.9} /> 思考过程 / Reasoning
              </div>
          ...
          {msg.decision && (
            <div className="bg-black/20 rounded-lg p-3">
              <div className="flex items-center gap-1.5 text-xs text-green-400 font-medium mb-2">
                <HugeiconsIcon icon={ChartBarLineIcon} size={13} strokeWidth={1.9} /> 交易决策 / Decision
              </div>
          ...
          {!parsed.reasoning && (
            <div className="bg-black/20 rounded-lg p-3">
              <div className="flex items-center gap-1.5 text-xs text-gray-400 font-medium mb-2">
                <HugeiconsIcon icon={Robot01Icon} size={13} strokeWidth={1.9} /> 完整输出 / Full Output
              </div>
          ...
          {msg.decisions && msg.decisions.length > 1 && (
            <div className="bg-black/20 rounded-lg p-3">
              <div className="flex items-center gap-1.5 text-xs text-purple-400 font-medium mb-2">
                <HugeiconsIcon icon={Robot01Icon} size={13} strokeWidth={1.9} /> 多币种决策 ({msg.decisions.length})
              </div>
...
function PersonalitySelect({ value, onChange, language }: { value: DebatePersonality; onChange: (p: DebatePersonality) => void; language: string }) {
  ...
  return (
    <div ref={ref} className="relative w-1/2 min-w-[160px]">
      ...
      >
        <HugeiconsIcon icon={p.icon} size={13} strokeWidth={1.9} className="text-[var(--text-secondary)] shrink-0" />
        {language === 'zh' ? p.name : p.nameEn}
        <HugeiconsIcon icon={ArrowDown01Icon} size={12} strokeWidth={1.9} className="opacity-60 ml-auto shrink-0" />
      </button>
      {open && (
        <div className="absolute left-0 right-0 top-full mt-0.5 z-50 py-1 rounded-lg bg-[var(--surface-primary)] border border-[var(--panel-border)] shadow-xl">
          {(Object.entries(PERS) as [DebatePersonality, typeof p][]).map(([k, v]) => {
            return (
              ...
              >
                <HugeiconsIcon icon={v.icon} size={13} strokeWidth={1.9} className="text-[var(--text-secondary)]" />
  ...
  return (
    <div ref={ref} className={`relative ${className || 'w-full'}`}>
      ...
      >
        ...
        <HugeiconsIcon icon={ArrowDown01Icon} size={14} strokeWidth={1.9} className="opacity-60 shrink-0" />
      </button>
      {open && (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
      <div className="bg-[var(--surface-secondary)] shadow-xl rounded-xl w-full max-w-2xl mx-4 p-6 sm:p-8 debate-modal">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-bold gl-metal-text">{t('createDebate', language)}</h3>
          <button onClick={onClose} className="dash-close-btn"><HugeiconsIcon icon={Cancel01Icon} size={18} strokeWidth={2} /></button>
        </div>
            ...
            {participants.map((p, i) => (
              <div key={i} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-[var(--surface-secondary)]/50 border border-[var(--panel-border)] w-full">
                ...
                <button onClick={() => setParticipants(participants.filter((_, j) => j !== i))}
                  className="ml-auto text-[var(--binance-red)] hover:text-red-300 shrink-0"><HugeiconsIcon icon={Cancel01Icon} size={13} strokeWidth={2} /></button>
              </div>
            ))}
            <button onClick={addP} className="self-start inline-flex items-center gap-1 px-2 py-1 text-xs debate-accent hover:bg-[var(--debate-accent-bg)] rounded">
              <HugeiconsIcon icon={Add01Icon} size={13} strokeWidth={2} /> {t('addAI', language)}
            </button>
          </div>
        </div>
    ...
    <DeepVoidBackground className="flex overflow-hidden relative" style={{ height: 'calc(100vh - 64px)' }} contentDirection="row" disableAnimation>
      ...
      <div className="hidden md:flex w-56 flex-shrink-0 bg-[var(--surface-primary)]/80 backdrop-blur-md border-r debate-panel-border flex-col z-10">
        ...
        <button onClick={() => setShowCreate(true)}
          ...
          <HugeiconsIcon icon={Add01Icon} size={16} strokeWidth={2} /> {t('newDebate', language)}
        </button>
        ...
        <div className="px-2 py-1 text-[11px] uppercase tracking-wider text-[var(--text-secondary)] font-semibold flex items-center gap-1.5">
          <HugeiconsIcon icon={BubbleChatIcon} size={12} strokeWidth={1.9} className="debate-accent" />
        ...
        <div className="flex-1 border-t debate-panel-border mt-2 overflow-hidden flex flex-col">
          <div className="px-2 py-2 text-[11px] uppercase tracking-wider text-[var(--text-secondary)] font-semibold flex items-center gap-1.5">
            <HugeiconsIcon icon={UserGroupIcon} size={12} strokeWidth={1.9} className="text-[var(--binance-green)]" />
        ...
        {detail ? (
          <>
              ...
              {Object.keys(rounds).length === 0 ? (
                <div className="flex-1 flex items-center justify-center">
                  {detail.status === 'pending' ? (
                    <EmptyState compact icon={PlayIcon} title={t('clickToStart', language)} />
                  ) : detail.status === 'completed' ? (
                    <EmptyState compact icon={AlertCircleIcon} title={t('debateFailed', language)} />
                  ) : (
                    <div className="flex flex-col items-center justify-center text-[var(--text-secondary)]">
                      <Loader2 size={64} className="animate-spin text-[var(--debate-accent)] mb-4" strokeWidth={1.5} />
                      <div className="text-lg text-center max-w-md">{t('waitingAI', language)}</div>
                    </div>
                  )}
                </div>
              ) : (
                <>
                  ...
                  <div className="flex-1 overflow-y-auto p-4 border-r debate-panel-border dash-scroll">
                    <div className="mb-3 flex items-center gap-2.5">
                      <span className="dash-ico">
                        <HugeiconsIcon icon={BubbleChatIcon} size={15} strokeWidth={1.9} />
                      </span>
                      <h2 className="text-sm font-semibold uppercase tracking-wider gl-metal-shine" style={{ animationDelay: '-1.2s' }}>
                        {t('discussionRecords', language)}
                      </h2>
                    </div>
                    <div className="space-y-3">
                      {Object.entries(rounds).map(([round, msgs]) => (
                        <div key={round} className="gl-onyx-panel-b rounded-xl p-3 overflow-hidden">
                          <div className="text-xs debate-accent font-bold mb-2 uppercase tracking-wider tabular-nums">Round {round}</div>
                          <div className="space-y-2">
                  ...
                  {votes.length > 0 && (
                    <div className="w-[420px] flex-shrink-0 overflow-y-auto p-4 bg-[var(--surface-primary)]/30 backdrop-blur-sm dash-scroll">
                      <div className="mb-3 flex items-center gap-2.5">
                        <span className="dash-ico">
                          <HugeiconsIcon icon={JusticeScale01Icon} size={15} strokeWidth={1.9} />
                        </span>
                        <h2 className="text-sm font-semibold uppercase tracking-wider gl-metal-shine" style={{ animationDelay: '-2.4s' }}>
            ...
            {(decision || votes.length > 0) && (
              <div className="p-3 border-t debate-panel-border debate-consensus-bar backdrop-blur-md flex items-center gap-4 flex-shrink-0">
                <div className="flex items-center gap-2">
                  <span className="dash-ico">
                    <HugeiconsIcon icon={JusticeScale01Icon} size={16} strokeWidth={1.9} />
                  </span>
                  ...
                  {decision ? (
                    ...
                    <span className="flex items-center gap-1 px-2 py-1 rounded font-bold bg-[var(--surface-tertiary)] text-[var(--text-secondary)]">
                      <HugeiconsIcon icon={Clock01Icon} size={14} strokeWidth={2} /> VOTING...
                    </span>
                  )}
                </div>
                {decision && (
                ...
                {decision && !decision.executed && (decision.action === 'open_long' || decision.action === 'open_short') && (
                  <button onClick={() => setExecId(detail.id)}
                    ...
                    <HugeiconsIcon icon={FlashIcon} size={14} strokeWidth={2} /> {t('execute', language)}
                  </button>
                )}
                {decision?.executed && (
                  <span className="text-green-400 text-sm font-semibold inline-flex items-center gap-1">
                    <HugeiconsIcon icon={CheckmarkCircle01Icon} size={15} strokeWidth={2} /> {t('executed', language)}
                  </span>
                )}
              </div>
            )}
          ...
          <div className="flex-1 flex items-center justify-center">
            <EmptyState
              ...
              action={{ label: t('newDebate', language), onClick: () => setShowCreate(true) }}
            />
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
          <div className="bg-[var(--surface-secondary)]/90 backdrop-blur-xl rounded-xl w-full max-w-sm p-6 debate-modal">
            ...
            <div className="flex items-center gap-1.5 text-xs text-blue-300 bg-[var(--debate-accent-bg)] p-2 rounded mb-3">
              <HugeiconsIcon icon={AlertCircleIcon} size={14} strokeWidth={1.9} className="shrink-0" />
