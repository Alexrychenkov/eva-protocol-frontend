...
import { DashPage, SectionHead, StatCard, EmptyState } from '../components/dash/DashKit'
    ...
    <DashPage>
      ...
      >
        <div className="p-5 sm:p-7 flex flex-col sm:flex-row sm:items-center gap-5 sm:gap-6">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-2 text-[11px] font-semibold uppercase tracking-[0.3em]" style={{ color: 'var(--accent-primary)' }}>
              {t.subtitle}
            </p>
      ...
      >
        <div className="p-4 sm:p-5 flex flex-col md:flex-row gap-4">
          {/* Search */}
          <div className="relative flex-1 flex items-center rounded-xl px-3" style={{ background: 'var(--surface-primary)', border: '1px solid var(--panel-border)' }}>
            <HugeiconsIcon icon={Search01Icon} size={17} strokeWidth={1.9} className="shrink-0" style={{ color: 'var(--text-tertiary)' }} />
            <input
              type="text"
          ...
          {/* Category Filter */}
          <div className="flex gap-1.5 p-1 rounded-xl shrink-0" style={{ background: 'var(--surface-primary)', border: '1px solid var(--panel-border)' }}>
            {['all', 'popular', 'recent'].map((cat) => (
              <button
                key={cat}
                onClick={() => setSelectedCategory(cat)}
                className={`px-4 py-2 text-[11px] font-semibold uppercase tracking-wider rounded-lg transition-all relative overflow-hidden ${selectedCategory === cat
              ...
              >
                {selectedCategory === cat && (
                  <motion.div
                    ...
                    style={{ background: 'var(--surface-tertiary)', border: '1px solid var(--panel-border)' }}
      ...
      {isLoading && (
        <div className="flex flex-col items-center justify-center py-32 gap-4">
          <Loader2 className="w-9 h-9 animate-spin" style={{ color: 'var(--accent-primary)' }} />
              ...
              return (
                ...
                >
                  <div className="p-5 relative">
                    ...
                    <div className="flex justify-between items-start mb-5">
                      ...
                      <div className="text-[10px]">
                        {strategy.config_visible ? (
                          <div className="dash-chip" style={{ color: 'var(--binance-green)', borderColor: 'rgba(14,203,129,0.25)' }}>
                            <HugeiconsIcon icon={ViewIcon} size={12} strokeWidth={2} />
                            <span className="dash-chip-val" style={{ color: 'var(--binance-green)' }}>PUBLIC_ACCESS</span>
                          </div>
                        ) : (
                          <div className="dash-chip">
                            <HugeiconsIcon icon={ViewOffSlashIcon} size={12} strokeWidth={2} />
                    ...
                    <p className="text-xs mb-5 line-clamp-2 h-8 leading-relaxed" style={{ color: 'var(--text-secondary)' }}>
                    ...
                    <div className="grid grid-cols-2 gap-y-2 mb-5 text-[10px]">
                      <div className="flex flex-col">
                        <span className="uppercase tracking-wider" style={{ color: 'var(--text-tertiary)' }}>{t.author}</span>
                        <span style={{ color: 'var(--text-secondary)' }}>@{strategy.author_email?.split('@')[0] || 'UNKNOWN'}</span>
                      </div>
                      <div className="flex flex-col text-right">
                        <span className="uppercase tracking-wider" style={{ color: 'var(--text-tertiary)' }}>{t.createdAt}</span>
                        <span className="tabular-nums" style={{ color: 'var(--text-secondary)' }}>{formatDate(strategy.created_at)}</span>
                      </div>
                    </div>

                    {/* Config / Indicators */}
                    <div className="rounded-xl p-3 mb-4 min-h-[90px]" style={{ background: 'var(--surface-primary)', border: '1px solid var(--panel-border)' }}>
                      {strategy.config_visible && strategy.config ? (
                        <div className="space-y-3">
                          ...
                          <div className="flex items-center gap-2 overflow-x-auto dash-scroll pb-1 flex-wrap">
                            ...
                            )) : <span className="text-[9px]" style={{ color: 'var(--text-tertiary)' }}>NO_INDICATORS</span>}
                          </div>
                          ...
                          {strategy.config.risk_control && (
                            <div className="flex justify-between items-center text-[10px]">
                              <div className="flex gap-3">
                                <div className="flex flex-col">
                                  <span className="scale-90 origin-left" style={{ color: 'var(--text-tertiary)' }}>LEV</span>
                                  <span className="font-bold tabular-nums" style={{ color: 'var(--text-primary)' }}>{strategy.config.risk_control.btc_eth_max_leverage || '-'}x</span>
                                </div>
                                <div className="flex flex-col">
                                  <span className="scale-90 origin-left" style={{ color: 'var(--text-tertiary)' }}>POS</span>
                                  <span className="font-bold tabular-nums" style={{ color: 'var(--text-primary)' }}>{strategy.config.risk_control.max_positions || '-'}</span>
                                </div>
                              </div>
                              <HugeiconsIcon icon={Analytics02Icon} size={13} strokeWidth={1.9} style={{ color: 'var(--text-tertiary)' }} />
                            </div>
                          )}
                        </div>
                      ) : (
                        <div className="flex flex-col items-center justify-center h-full" style={{ color: 'var(--text-tertiary)' }}>
                          <HugeiconsIcon icon={ViewOffSlashIcon} size={17} strokeWidth={1.8} className="mb-1 opacity-60" />
                      ...
                      {strategy.config_visible && strategy.config ? (
                        ...
                        >
                          {copiedId === strategy.id ? (
                            <>
                              <HugeiconsIcon icon={Tick02Icon} size={14} strokeWidth={2.2} style={{ color: 'var(--binance-green)' }} />
                              <span style={{ color: 'var(--binance-green)' }}>{t.copied}</span>
                            </>
                          ) : (
                            <>
                        ...
                        <button disabled className="w-full py-2.5 rounded-xl text-[11px] font-bold uppercase tracking-widest cursor-not-allowed flex items-center justify-center gap-2" style={{ background: 'var(--surface-primary)', border: '1px solid var(--panel-border)', color: 'var(--text-tertiary)' }}>
                          <HugeiconsIcon icon={Shield01Icon} size={13} strokeWidth={1.9} />
                          {t.hideConfig}
                        </button>
                      )}
                    </div>
          ...
          >
            <div className="relative px-6 sm:px-8 py-5 flex items-center gap-4">
              ...
              <div className="text-left min-w-0 flex-1">
                <div className="text-sm font-bold uppercase tracking-wider gl-metal-text">{t.shareYours}</div>
                <div className="text-[10px] mt-0.5" style={{ color: 'var(--text-secondary)' }}>
                  {language === 'zh' ? '为全球策略库贡献你的配置' : 'CONTRIBUTE TO THE GLOBAL DATABASE'}
                </div>
              </div>
              <HugeiconsIcon icon={ArrowRight01Icon} size={20} strokeWidth={2} className="shrink-0 group-hover:translate-x-1 transition-transform" style={{ color: 'var(--accent-primary)' }} />
            </div>
          </div>
        </motion.div>
      )}
