...
export function UpgradePage() {
  ...
  const gateHeadline = gate.status === 'eligible'
    ...
    : gate.status === 'error'
      ? isEn ? 'Unable to verify EVA balance' : '无法验证 EVA 余额'
      : gate.status === 'checking'
        ? isEn ? 'Checking EVA balance…' : '正在检查 EVA 余额…'
        : gate.status === 'disconnected'
          ? isEn ? 'Connect a wallet to check eligibility' : '连接钱包以检查资格'
          : isEn ? 'Need more EVA to unlock' : '需要更多 EVA 才能解锁'
  ...
  const gateSubcopy = gate.status === 'unconfigured'
    ...
    : gate.status === 'error'
      ? isEn
        ? gate.error || 'The app could not read your Solana EVA balance. Make sure the connected wallet is your Solana wallet, then try again.'
        : gate.error || '应用无法读取您的 Solana EVA 余额。请确认当前连接的是持有 EVA 的 Solana 钱包，然后重试。'
      ...
      <main className="pt-16 relative z-10">
        <section className="relative overflow-hidden px-4 py-20 sm:py-28">
          <div
            ...
            style={{
              background: 'radial-gradient(ellipse 50% 32% at 50% 2%, rgba(61,107,255,0.09) 0%, transparent 58%)',
            }}
          />
          ...
          <div className="max-w-6xl mx-auto relative z-10 space-y-8">
            <motion.div
              ...
              className="max-w-3xl"
            >
              <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold tracking-tight whitespace-nowrap" style={{ letterSpacing: '-0.04em' }}>
                {isEn ? (
                  <>Upgrade unlocks the next layer of <span className="eva-shimmer">EVA</span></>
                ) : (
                  <>Upgrade 解锁 <span className="eva-shimmer">EVA</span> 的下一层能力</>
                )}
              </h1>
              <p className="text-lg mt-5 leading-relaxed whitespace-nowrap" style={{ color: 'var(--text-secondary)' }}>
                {isEn
              ...
              <div className="rounded-[28px] p-6 sm:p-7 overflow-hidden gl-metal-panel">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <h2 className="text-2xl font-semibold gl-metal-text">{gateHeadline}</h2>
                  </div>
                  <div
                    className="rounded-full px-3 py-1.5 text-xs flex items-center gap-1.5"
                    style={{
                      background: gate.isEligible ? 'rgba(16,203,129,0.10)' : 'rgba(255,255,255,0.035)',
                      border: `1px solid ${gate.isEligible ? 'rgba(16,203,129,0.30)' : 'rgba(255,255,255,0.10)'}`,
                      boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.05)',
                    }}
                  >
                    <span className={`gl-gate-label${gate.isEligible ? ' gl-gate-label--unlocked' : ''}`}>
...
function MetricCard({
  ...
  return (
    <div className="rounded-2xl px-4 py-3 overflow-hidden gl-onyx-panel">
      ...
      <div className={`${compact ? 'text-sm' : 'text-lg'} font-semibold mt-2 break-words gl-metal-text`}>
        {value}
      </div>
    </div>
  )
