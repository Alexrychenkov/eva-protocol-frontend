...
import { Wallet as PhWallet, CheckCircle as PhCheck } from '@phosphor-icons/react'
...
export function LandingPage() {
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 pointer-events-none" style={{ background: '#050505' }} />

      {/* ── IDLE: ghostlink noir terminal ── */}
      {phase === 'idle' && (
        <div
          className="fixed inset-0 flex flex-col items-center justify-center"
          style={{ paddingTop: '64px', opacity: sceneRevealed ? 1 : 0, transition: 'opacity 0.9s ease' }}
        >

          {/* Soft center glow (the 3D canvas adds its own CenterGlowPlane behind the coins) */}
          <div className="absolute inset-0 pointer-events-none gl-center-glow" style={{ zIndex: 0 }} />
            ...
            <div className="fixed inset-0 pointer-events-none overflow-hidden" style={{ zIndex: 0 }}>
              <ErrorBoundary name="darkveil" silent>
                <div
                  ...
                  style={{ filter: 'hue-rotate(-56deg) saturate(1.25) brightness(1.0)' }}
                >
                  <DarkVeil
                    hueShift={0}
                    speed={0.42}
                ...
                <div
                  className="absolute inset-0"
                  style={{ background: 'radial-gradient(130% 105% at 50% 2%, rgba(4,6,18,0) 0%, rgba(4,6,18,0.18) 45%, rgba(4,6,18,0.6) 88%)' }}
                />
              </ErrorBoundary>
            </div>
          )}
          ...
          <div
            ...
            style={{
              ...
              width: 440,
              height: 440,
              background: 'radial-gradient(circle, rgba(180,200,255,0.10) 0%, rgba(255,255,255,0.04) 45%, transparent 70%)',
              filter: 'blur(55px)',
          ...
          <div className="absolute inset-0 pointer-events-none">
            <div className="absolute top-0 left-0 w-24 h-px" style={{ background: 'linear-gradient(to right, rgba(255,255,255,0.18), transparent)' }} />
            <div className="absolute top-0 left-0 w-px h-24" style={{ background: 'linear-gradient(to bottom, rgba(255,255,255,0.18), transparent)' }} />
            <div className="absolute bottom-0 right-0 w-24 h-px" style={{ background: 'linear-gradient(to left, rgba(255,255,255,0.18), transparent)' }} />
            <div className="absolute bottom-0 right-0 w-px h-24" style={{ background: 'linear-gradient(to top, rgba(255,255,255,0.18), transparent)' }} />
          </div>
          ...
          >
            ...
            >
              <span
                ...
                style={{ fontSize: isMobile ? '1.4rem' : '1.9rem', letterSpacing: '0.22em' }}
              >
                EVA PROTOCOL
              </span>
            </motion.div>
            ...
            >
                ...
                <div className="relative flex-1">
                  ...
                  <button
                    ...
                    style={{
                      ...
                      border: input.trim() ? '1px solid var(--accent-primary-border-strong)' : 'none',
                      color: input.trim() ? '#fff' : 'rgba(255,255,255,0.7)',
                      boxShadow: input.trim() ? '0 0 22px rgba(61,107,255,0.45)' : 'none',
                    }}
                    aria-label={input.trim() ? 'Send' : 'Commands'}
                  >
                    {input.trim() ? <ArrowRight className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
                  </button>
                </div>
              ...
              {panelOpen && (
                <div
                  ...
                  style={{
                    background: 'linear-gradient(180deg, #14151a 0%, #090a0d 100%)',
                    border: '1px solid rgba(255,255,255,0.10)',
                    borderTop: '1px solid rgba(255,255,255,0.16)',
                    boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.06), 0 28px 70px rgba(0,0,0,0.9)',
                  }}
                >
                  <div className="px-4 pt-3 pb-1 text-[10px] uppercase tracking-[0.22em] font-black" style={{ color: 'rgba(255,255,255,0.45)' }}><span style={{ color: 'var(--accent-primary)' }}>/</span> Commands</div>
                  <div className="p-2 flex flex-col gap-0.5">
                    <button
                      ...
                      onMouseEnter={(e) => { e.currentTarget.style.background = 'rgba(255,255,255,0.06)'; e.currentTarget.style.color = '#9fb4ff' }}
                      onMouseLeave={(e) => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'var(--text-secondary)' }}
                    >
                      <BookUser className="w-4 h-4 shrink-0" style={{ color: 'var(--accent-primary)' }} />
                      <span className="flex-1">Contacts</span>
                      <span className="text-[10px] font-mono font-black" style={{ color: 'rgba(135,160,255,0.7)' }}>/addr</span>
                    </button>
                    {COMMANDS.map((cmd) => (
                      <button
                        key={cmd.hint}
                        ...
                        onMouseLeave={(e) => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'var(--text-secondary)' }}
                      >
                        <ArrowRight className="w-4 h-4 shrink-0" style={{ color: 'var(--accent-primary)' }} />
                        <span className="flex-1">{cmd.label}</span>
                        <span className="text-[10px] font-mono font-black" style={{ color: 'rgba(135,160,255,0.7)' }}>{cmd.hint}</span>
                      </button>
                    ))}
                  </div>
                  <div className="pb-2" />
      ...
      {phase === 'chat' && (
        <div className="fixed inset-0 flex flex-col overflow-x-hidden" style={{ paddingTop: '64px' }}>
          ...
          <div className="relative z-10 flex flex-col flex-1 overflow-hidden max-w-2xl w-full mx-auto px-3 sm:px-6">
            ...
            {/* Messages */}
            <div ref={messagesContainerRef} className="flex-1 overflow-y-auto overflow-x-hidden pb-4 space-y-4 pr-1 pl-1 scrollbar-hide">
              {messages.filter((msg) => !(msg.role === 'assistant' && msg.content === '')).map((msg) => (
                <div key={msg.id} className={`flex animate-fade-in group/turn ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  {msg.role === 'system' ? (
                    ...
                    <div className={`max-w-[85%] overflow-visible ${msg.swapIntent ? 'sm:min-w-[320px]' : ''}`}>
