...
const TICKER_CG_IDS: Record<string, string> = {
  ...
  LINKUSDT: 'chainlink', DOTUSDT: 'polkadot', POLUSDT: 'polygon-ecosystem-token', UNIUSDT: 'uniswap',
  ...
  TIAUSDT: 'celestia', JUPUSDT: 'jupiter', WIFUSDT: 'dogwifcoin', BONKUSDT: 'bonk', PEPEUSDT: 'pepe',
}
// Split into two batches to avoid CoinGecko free-tier rate limits on large requests
const TICKER_CG_BATCH1 = TICKER_SYMBOLS.slice(0, 13).map(s => TICKER_CG_IDS[s]).join(',')
const TICKER_CG_BATCH2 = TICKER_SYMBOLS.slice(13).map(s => TICKER_CG_IDS[s]).join(',')
...
function TickerBar() {
  const updateDOM = useCallback((sym: string, data: TickerData) => {
    ...
    ;[`ticker-${sym}-a`, `ticker-${sym}-b`].forEach(id => {
      ...
      if (pctEl) {
        ...
        pctEl.style.color = data.changePercent.startsWith('+') ? '#10b981' : '#ef4444'
      }
    })
  }, [])
  ...
  useEffect(() => {
    const fetchTicker = async () => {
      try {
        ...
        const res = await fetch(`/binance/api/v3/ticker/24hr?symbols=${encodeURIComponent(syms)}`)
  ...
  return (
    <div
      ...
      style={{ borderColor: 'rgba(255,255,255,0.06)', background: 'linear-gradient(180deg, rgba(20,24,33,0.72) 0%, rgba(11,13,19,0.72) 100%)', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.04), 0 1px 0 rgba(0,0,0,0.5)' }}
    >
...
function ConverterWidget({ coins }: { coins: CoinRow[]; isEn: boolean }) {
  ...
  const toOptions: TokenOption[] = [{ value: 'usd', label: 'USD' }, ...tokenOpts]
    ...
    <div className="p-4 flex flex-col gap-2.5">
      ...
      <div className="flex gap-2">
        ...
        <div className="w-[104px] shrink-0">
          <TokenSelect value={fromId} onChange={setFromId} options={fromOptions} ariaLabel="From token" />
        </div>
      </div>
      {/* Swap */}
      <div className="flex items-center justify-center">
        <span className="text-base leading-none" style={{ color: 'var(--accent-primary)' }}>⇅</span>
      </div>
      {/* To */}
      <div className="flex gap-2">
        <div
        ...
        <div className="w-[104px] shrink-0">
          <TokenSelect value={toId} onChange={setToId} options={toOptions} ariaLabel="To token" />
        </div>
      </div>
    </div>
  )
...
export function DataPage() {
  ...
  const fetchGlobal = useCallback(async () => {
    try {
      ...
      const [globalJson, fngJson] = await Promise.all([
        ...
        fetch('/feargreed/fng?limit=1').then(r => r.json()).catch(() => null),
      ]) as [{ data?: Record<string, unknown> }, { data?: Array<{ value: string; value_classification: string }> } | null]
  ...
  const fetchCoins = useCallback(async () => {
    try {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const json = await cgFetch('/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=50&page=1&sparkline=false&price_change_percentage=7d,24h') as any[]
      if (!Array.isArray(json)) return
      setCoins(json.slice(0, 20).map(c => ({
        id: c.id,
      ...
      setGainers(sorted.slice(0, 5).map(c => ({
        id: c.id,
        name: c.name,
        symbol: c.symbol.toUpperCase(),
        image: c.image,
  ...
  const fetchTrending = useCallback(async () => {
    try {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      setTrending((json.coins as any[]).slice(0, 7).map((c: any) => ({
        id: c.item.id,
        name: c.item.name,
  ...
  const fetchChart = useCallback(async () => {
    try {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const json = await cgFetch('/api/v3/coins/bitcoin/market_chart?vs_currency=usd&days=7&interval=hourly') as any
      if (!json?.market_caps) return
      const mcPts: number[] = (json.market_caps ?? []).map((p: [number, number]) => p[1])
      if (mcPts.length >= 2) setMarketCapPoints(mcPts)
      const volPts: number[] = (json.total_volumes ?? []).slice(-24).map((p: [number, number]) => p[1])
  ...
  return (
    <div className="min-h-screen relative gl-data-page">
      ...
      <svg width="0" height="0" aria-hidden="true" style={{ position: 'absolute', pointerEvents: 'none' }}>
        <filter id="gl-smoke" x="-40%" y="-40%" width="180%" height="180%" colorInterpolationFilters="sRGB">
        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 py-8 space-y-8">
          ...
          >
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div
                  className="rounded-xl p-4 gl-onyx-panel"
                  style={{}}
                >
            ...
            >
              <div className="flex justify-between items-center pt-3.5 mb-2 px-4">
                <span className="font-semibold text-sm gl-metal-shine" style={{ animationDelay: '-1.2s' }}>
                  {isEn ? 'Trending' : '热门'}
                </span>
              </div>
              <div>
                {trending.length === 0
                  ...
                  : trending.slice(0, 5).map(coin => (
                    <div
                      ...
                      onMouseEnter={e => (e.currentTarget.style.background = 'rgba(70,100,180,0.10)')}
            ...
            >
              <div className="flex justify-between items-center pt-3.5 mb-2 px-4">
                <span className="font-semibold text-sm gl-metal-shine" style={{ animationDelay: '-2.4s' }}>
          ...
          >
              ...
              >
                <div className="px-4 py-3 border-b" style={{ borderColor: 'var(--panel-border)' }}>
                  <span className="text-xs font-semibold uppercase tracking-wider gl-metal-shine" style={{ animationDelay: '-3.6s' }}>
              ...
              <div
                ...
                style={{ border: '1px solid var(--panel-border)', boxShadow: '0 2px 4px rgba(0,0,0,0.3), 0 12px 32px rgba(0,0,0,0.4)' }}
              >
                ...
                <div className="px-4 py-3 border-b" style={{ borderColor: 'rgba(255,255,255,0.10)' }}>
                  <span className="text-xs font-semibold uppercase tracking-wider gl-metal-shine" style={{ animationDelay: '-4.8s' }}>
                    {isEn ? 'Converter' : '换算器'}
                  </span>
                </div>
          ...
          >
            ...
            >
              ...
              >
                <h2 className="text-sm font-semibold gl-metal-shine" style={{ animationDelay: '-6s' }}>
