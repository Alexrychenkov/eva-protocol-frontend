...
import { ChartLineData01Icon, ChartCandlestickIcon, ArrowDown01Icon, Search01Icon, Diamond01Icon, Bitcoin01Icon, ChartUpIcon, Money03Icon, GoldIcon } from '@hugeicons/core-free-icons'
...
// 市场类型配置
const MARKET_CONFIG = {
  hyperliquid: { exchange: 'hyperliquid', defaultSymbol: 'BTC', icon: Diamond01Icon, label: { zh: 'HL', en: 'HL' }, color: 'cyan', hasDropdown: true },
  crypto: { exchange: 'binance', defaultSymbol: 'BTCUSDT', icon: Bitcoin01Icon, label: { zh: '加密', en: 'Crypto' }, color: 'accent', hasDropdown: false },
  stocks: { exchange: 'alpaca', defaultSymbol: 'AAPL', icon: ChartUpIcon, label: { zh: '美股', en: 'Stocks' }, color: 'green', hasDropdown: false },
  forex: { exchange: 'forex', defaultSymbol: 'EUR/USD', icon: Money03Icon, label: { zh: '外汇', en: 'Forex' }, color: 'blue', hasDropdown: false },
  metals: { exchange: 'metals', defaultSymbol: 'XAU/USD', icon: GoldIcon, label: { zh: '金属', en: 'Metals' }, color: 'accent', hasDropdown: false },
}
...
export function ChartTabs({ traderId, selectedSymbol, updateKey, exchangeId }: ChartTabsProps) {
  ...
  return (
    <div className={`gl-onyx-panel rounded-2xl overflow-hidden relative z-10 w-full flex flex-col transition-all duration-300 ${typeof window !== 'undefined' && window.innerWidth < 768 ? 'h-[500px]' : 'h-[600px]'
      }`}>
      {/*
      ...
      */}
      <div
        ...
        style={{ borderBottom: '1px solid var(--panel-border)', background: 'linear-gradient(180deg, rgba(20,26,38,0.55), rgba(11,14,17,0.35))' }}
      >
        {/* Left: Tab Switcher */}
        <div className="flex flex-wrap items-center gap-2">
          <div className="gl-seg">
            ...
            >
              <HugeiconsIcon icon={ChartLineData01Icon} size={15} strokeWidth={1.9} />
            ...
            <button
              ...
              className="gl-seg-item flex items-center gap-1.5"
            >
              <HugeiconsIcon icon={ChartCandlestickIcon} size={15} strokeWidth={1.9} />
              ...
              {(Object.keys(MARKET_CONFIG) as MarketType[]).map((type) => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wide rounded-lg transition-all border active:scale-[0.97] ${isActive
                  ...
                  >
                    <HugeiconsIcon icon={config.icon} size={13} strokeWidth={1.9} className={isActive ? 'text-[var(--accent-primary)]' : 'opacity-70'} />
        ...
        {activeTab === 'kline' && (
          <div className="flex items-center gap-2 md:gap-3 w-full md:w-auto min-w-0">
              ...
              {marketConfig.hasDropdown ? (
                <>
                  ...
                  >
                    ...
                    <HugeiconsIcon icon={ArrowDown01Icon} size={13} strokeWidth={2} className={`text-[var(--text-tertiary)] transition-transform ${showDropdown ? 'rotate-180' : ''}`} />
                  </button>
                  {showDropdown && (
                    <div className="gl-onyx-panel-b absolute top-full right-0 mt-2 w-64 rounded-xl shadow-[0_18px_48px_-12px_rgba(0,0,0,0.65)] z-50 overflow-hidden">
                      <div className="p-2 border-b border-[var(--panel-border)]">
                        <div className="flex items-center gap-2 px-2.5 py-1.5 bg-[var(--surface-tertiary)] rounded-lg border border-[var(--panel-border)] focus-within:border-[rgba(61,107,255,0.5)] focus-within:shadow-[0_0_12px_rgba(61,107,255,0.18)] transition-all">
                          <HugeiconsIcon icon={Search01Icon} size={14} strokeWidth={1.9} className="text-[var(--text-tertiary)]" />
                          <input
                            type="text"
                            value={searchFilter}
                      ...
                      <div className="overflow-y-auto max-h-60 dash-scroll">
                        {['crypto', 'stock', 'forex', 'commodity', 'index'].map(category => {
                          ...
                          return (
                            <div key={category}>
                              <div className="px-3 py-1.5 text-[9px] font-bold text-[var(--text-tertiary)] bg-white/[0.03] uppercase tracking-wider">{labels[category]}</div>
                              {categorySymbols.map(s => (
                                ...
                                >
                                  <span>{s.symbol}</span>
                                  <span className="text-[9px] opacity-50">{s.name}</span>
                                </button>
                              ))}
                            </div>
                          )
                ...
                <span className="px-2.5 py-1.5 bg-[var(--surface-tertiary)] border border-[var(--panel-border)] rounded-lg text-[11px] font-bold gl-metal-text font-mono tabular-nums">{chartSymbol}</span>
              )}
            </div>
            ...
            <div className="flex items-center gap-0.5 bg-[var(--surface-tertiary)] rounded-lg border border-[var(--panel-border)] p-0.5 overflow-x-auto no-scrollbar max-w-[200px] md:max-w-none">
              {INTERVALS.map((int) => (
                <button
                  ...
                  className={`px-2 py-1 text-[10px] font-semibold tabular-nums rounded-md transition-all active:scale-[0.95] ${interval === int.value
            ...
            <form onSubmit={handleSymbolSubmit} className="hidden md:flex items-center shrink-0">
              ...
              />
              <button type="submit" className="px-2.5 py-1.5 bg-white/[0.04] border border-[var(--panel-border)] border-l-0 rounded-r-lg text-[10px] font-semibold uppercase tracking-wide text-[var(--text-tertiary)] hover:text-[var(--text-primary)] hover:bg-white/[0.08] transition-all active:scale-[0.97]">
                Go
              </button>
            </form>
          </div>
      ...
      <div className="relative flex-1 overflow-hidden h-full min-h-0" style={{ background: 'linear-gradient(180deg, rgba(11,14,17,0.35), rgba(8,10,14,0.55))' }}>
