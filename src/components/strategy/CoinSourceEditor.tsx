...
export function CoinSourceEditor({
  ...
  const t = (key: string) => {
    const translations: Record<string, Record<string, string>> = {
      ...
      evaDataNote: { zh: '使用 EVA API Key（在指标配置中设置）', en: 'Uses EVA API Key (set in Indicators config)' },
    }
  ...
  // EVA badge component
  const NofxOSBadge = () => (
    <span className="gl-badge gl-badge--info text-[9px]">
      EVA
    </span>
  )
    ...
    <div className="space-y-6">
          ...
          {sourceTypes.map(({ value, icon }) => {
            ...
            return (
              ...
              >
                ...
                <div className={`text-sm font-semibold ${active ? 'gl-metal-text' : 'text-[var(--text-primary)]'}`}>
                  {t(value)}
                </div>
      ...
      <div className="gl-onyx-panel-b rounded-2xl p-4 sm:p-5 overflow-hidden">
        <div className="flex items-center gap-2.5 mb-2">
          ...
          <label className="text-sm font-semibold uppercase tracking-wider gl-metal-shine" style={{ animationDelay: '-2.4s' }}>
            {t('excludedCoins')}
          </label>
        </div>
        ...
        <div className="flex flex-wrap gap-2 mb-3">
          {(config.excluded_coins || []).map((coin) => (
            <span
              key={coin}
              className="dash-chip flex items-center gap-1.5"
              style={{ borderColor: 'color-mix(in srgb, var(--binance-red) 32%, var(--panel-border))' }}
            >
              <span className="dash-chip-val tabular-nums" style={{ color: 'var(--binance-red)' }}>{coin}</span>
              {!disabled && (
                <button
        ...
        {!disabled && (
          <div className="flex gap-2">
            ...
            >
              <HugeiconsIcon icon={UnavailableIcon} size={15} strokeWidth={1.9} />
              {t('addExcludedCoin')}
            </button>
          </div>
        )}
      ...
      {config.source_type === 'ai500' && (
        <div className="gl-onyx-panel rounded-2xl p-4 sm:p-5 overflow-hidden">
          <div className="flex items-center gap-2.5 mb-4">
            ...
            <span className="text-sm font-semibold uppercase tracking-wider gl-metal-shine" style={{ animationDelay: '-1.2s' }}>
          ...
          <div className="space-y-4">
            ...
            >
              <span className="text-sm font-medium text-[var(--text-primary)]">{t('useAI500')}</span>
              <span className="gl-switch shrink-0" data-on={config.use_ai500 ? 'true' : 'false'} aria-hidden />
            </label>
      ...
      {config.source_type === 'oi_top' && (
        <div className="gl-onyx-panel rounded-2xl p-4 sm:p-5 overflow-hidden">
          <div className="flex items-center gap-2.5 mb-4">
            <span className="dash-ico" style={{ color: 'var(--binance-green)' }}>
          ...
          <div className="space-y-4">
            ...
            >
              <span className="text-sm font-medium text-[var(--text-primary)]">{t('useOITop')}</span>
              <span className="gl-switch shrink-0" data-on={config.use_oi_top ? 'true' : 'false'} aria-hidden />
            </label>
      ...
      {config.source_type === 'oi_low' && (
        <div className="gl-onyx-panel rounded-2xl p-4 sm:p-5 overflow-hidden">
          ...
          <div className="space-y-4">
            ...
            >
              <span className="text-sm font-medium text-[var(--text-primary)]">{t('useOILow')}</span>
              <span className="gl-switch shrink-0" data-on={config.use_oi_low ? 'true' : 'false'} aria-hidden />
            </label>
      ...
      {config.source_type === 'mixed' && (
        <div className="gl-aurora-panel rounded-2xl p-4 sm:p-5 overflow-hidden">
          <div className="flex items-center gap-2.5 mb-4">
            ...
            <span className="text-sm font-semibold uppercase tracking-wider gl-metal-shine" style={{ animationDelay: '-3.6s' }}>
              {t('mixedConfig')}
            </span>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                <span className="dash-ico" style={{ width: 26, height: 26 }}>
                  <HugeiconsIcon icon={DatabaseIcon} size={14} strokeWidth={1.9} />
                </span>
                <span className="text-sm font-semibold gl-metal-text">AI500</span>
                <NofxOSBadge />
                <span
              ...
              {config.use_ai500 && (
                <div className="flex items-center gap-2 mt-3 pl-9">
                  <span className="gl-field-label !mb-0 !text-[10px]">Limit</span>
                  <select
                    value={config.ai500_limit || 10}
                    onChange={(e) => {
                      e.stopPropagation()
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                <span className="dash-ico" style={{ width: 26, height: 26, color: 'var(--binance-green)' }}>
                  <HugeiconsIcon icon={TradeUpIcon} size={14} strokeWidth={1.9} />
                </span>
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                <span className="dash-ico" style={{ width: 26, height: 26, color: 'var(--binance-red)' }}>
                  <HugeiconsIcon icon={TradeDownIcon} size={14} strokeWidth={1.9} />
                </span>
                <span className="text-sm font-semibold gl-metal-text">
            ...
            {/* Static/Custom Card */}
            <div
              data-active={(config.static_coins || []).length > 0 ? 'true' : 'false'}
              className={`gl-onyx-panel rounded-xl p-3.5 transition-all overflow-hidden ${
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                <span className="dash-ico" style={{ width: 26, height: 26 }}>
                  <HugeiconsIcon icon={ListViewIcon} size={14} strokeWidth={1.9} />
                </span>
                <span className="text-sm font-semibold gl-metal-text">
              ...
              <div className="flex flex-wrap gap-1.5 mt-2">
                {(config.static_coins || []).slice(0, 3).map((coin) => (
                  ...
                  >
                    <span className="dash-chip-val !text-xs tabular-nums">{coin}</span>
                    {!disabled && (
                      <button
                        onClick={(e) => {
                          e.stopPropagation()
            ...
            return (
              <div className="gl-prism-panel rounded-xl p-3 overflow-hidden">
                <div className="flex items-center justify-between gap-3 text-xs">
                  <span className="gl-field-label !mb-0">{t('mixedSummary')}</span>
                  <span className="gl-metal-text font-semibold tabular-nums text-right">
                    {sources.join(' + ')}
                  </span>
                </div>
                <div className="text-[11px] text-[var(--text-secondary)] mt-1.5">
                  {t('maxCoins')} <span className="tabular-nums text-[var(--text-primary)] font-semibold">{totalLimit}</span> {t('coins')}
                </div>
              </div>
            )
          })()}
