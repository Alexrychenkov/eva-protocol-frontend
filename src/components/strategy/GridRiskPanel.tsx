    ...
    return (
      <div className="gl-panel rounded-xl overflow-hidden animate-pulse">
        <div className="flex items-center gap-2.5 px-4 py-3.5">
          ...
          <div className="h-3 w-24 rounded-full" style={{ background: 'var(--surface-tertiary)' }} />
        </div>
      </div>
    )
  }
      ...
      >
        ...
        <div className="flex items-center gap-2">
          ...
          <span
            ...
            style={{ width: 26, height: 26, background: 'var(--surface-tertiary)', color: 'var(--text-secondary)' }}
          >
            <HugeiconsIcon icon={expanded ? ArrowUp01Icon : ArrowDown01Icon} size={15} strokeWidth={2} />
          </span>
        </div>
      </div>
      ...
      {expanded && (
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="gl-onyx-panel rounded-xl p-3 overflow-hidden">
              <div className="flex items-center gap-1.5 mb-2.5">
                <HugeiconsIcon icon={TradeUpIcon} size={14} strokeWidth={1.9} style={{ color: 'var(--accent-primary)' }} />
                <span className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: 'var(--text-secondary)' }}>{t('leverageInfo')}</span>
              </div>
              <div className="grid grid-cols-3 gap-1.5 text-xs">
                <div>
                  <div className="text-[10px] uppercase tracking-wide mb-0.5" style={{ color: 'var(--text-tertiary)' }}>{t('currentLeverage')}</div>
                  <div className="font-bold tabular-nums gl-metal-text">{riskInfo.current_leverage}x</div>
                </div>
                <div>
                  <div className="text-[10px] uppercase tracking-wide mb-0.5" style={{ color: 'var(--text-tertiary)' }}>{t('effectiveLeverage')}</div>
                  <div className="font-bold tabular-nums" style={{ color: 'var(--accent-primary)' }}>{riskInfo.effective_leverage.toFixed(2)}x</div>
                </div>
                <div>
                  <div className="text-[10px] uppercase tracking-wide mb-0.5" style={{ color: 'var(--text-tertiary)' }}>{t('recommendedLeverage')}</div>
                  <div
            ...
            <div className="gl-onyx-panel rounded-xl p-3 overflow-hidden">
              <div className="flex items-center gap-1.5 mb-2.5">
                <HugeiconsIcon icon={Activity01Icon} size={14} strokeWidth={1.9} style={{ color: 'var(--accent-primary)' }} />
                <span className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: 'var(--text-secondary)' }}>{t('positionInfo')}</span>
              </div>
              <div className="grid grid-cols-3 gap-1.5 text-xs">
                <div>
                  <div className="text-[10px] uppercase tracking-wide mb-0.5" style={{ color: 'var(--text-tertiary)' }}>{t('currentPosition')}</div>
                  <div className="font-bold tabular-nums gl-metal-text">{formatUSD(riskInfo.current_position)}</div>
                </div>
                <div>
                  <div className="text-[10px] uppercase tracking-wide mb-0.5" style={{ color: 'var(--text-tertiary)' }}>{t('maxPosition')}</div>
                  <div className="font-bold tabular-nums gl-metal-text">{formatUSD(riskInfo.max_position)}</div>
                </div>
                <div>
                  <div className="text-[10px] uppercase tracking-wide mb-0.5" style={{ color: 'var(--text-tertiary)' }}>{t('positionPercent')}</div>
                  <div className="font-bold tabular-nums" style={{ color: getPositionColor(riskInfo.position_percent) }}>
                    {riskInfo.position_percent.toFixed(1)}%
                  </div>
                </div>
              </div>
              {/* Mini progress bar */}
              <div className="h-1.5 mt-2.5 rounded-full overflow-hidden" style={{ background: 'var(--surface-tertiary)', boxShadow: 'inset 0 1px 2px rgba(0,0,0,0.4)' }}>
                <div
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="gl-onyx-panel rounded-xl p-3 overflow-hidden">
              <div className="flex items-center gap-1.5 mb-2.5">
                <HugeiconsIcon icon={GaugeIcon} size={14} strokeWidth={1.9} style={{ color: 'var(--accent-primary)' }} />
                <span className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: 'var(--text-secondary)' }}>{t('marketState')}</span>
              </div>
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div>
                  <div className="text-[10px] uppercase tracking-wide mb-0.5" style={{ color: 'var(--text-tertiary)' }}>{t('regimeLevel')}</div>
                  <div className="font-bold" style={{ color: getRegimeColor(riskInfo.regime_level) }}>
                    {t(riskInfo.regime_level || 'standard')}
                  </div>
                </div>
                <div>
                  <div className="text-[10px] uppercase tracking-wide mb-0.5" style={{ color: 'var(--text-tertiary)' }}>{t('currentPrice')}</div>
                  <div className="font-bold tabular-nums gl-metal-text">{formatPrice(riskInfo.current_price)}</div>
                </div>
                <div>
                  <div className="text-[10px] uppercase tracking-wide mb-0.5" style={{ color: 'var(--text-tertiary)' }}>{t('breakoutLevel')}</div>
                  <div className="font-bold" style={{ color: getBreakoutColor(riskInfo.breakout_level) }}>
                    {t(riskInfo.breakout_level || 'none')}
                  </div>
                </div>
                <div>
                  <div className="text-[10px] uppercase tracking-wide mb-0.5" style={{ color: 'var(--text-tertiary)' }}>{t('breakoutDirection')}</div>
                  <div
                  ...
                  >
                    {riskInfo.breakout_direction ? (
                      <HugeiconsIcon
                        icon={riskInfo.breakout_direction === 'up' ? ArrowUp01Icon : ArrowDown01Icon}
                        size={14}
                        strokeWidth={2.4}
                      />
                    ) : (
            ...
            <div className="gl-onyx-panel rounded-xl p-3 overflow-hidden">
              <div className="flex items-center gap-1.5 mb-2.5">
                <HugeiconsIcon icon={AlertCircleIcon} size={14} strokeWidth={1.9} style={{ color: 'var(--binance-red)' }} />
                <span className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: 'var(--text-secondary)' }}>{t('liquidationInfo')}</span>
              </div>
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div>
                  <div className="text-[10px] uppercase tracking-wide mb-0.5" style={{ color: 'var(--text-tertiary)' }}>{t('liquidationPrice')}</div>
                  <div className="font-bold tabular-nums" style={{ color: 'var(--binance-red)' }}>
                    {riskInfo.liquidation_price > 0 ? formatPrice(riskInfo.liquidation_price) : '-'}
                  </div>
                </div>
                <div>
                  <div className="text-[10px] uppercase tracking-wide mb-0.5" style={{ color: 'var(--text-tertiary)' }}>{t('liquidationDistance')}</div>
                  <div className="font-bold tabular-nums" style={{ color: 'var(--binance-red)' }}>
          ...
          <div className="gl-onyx-panel rounded-xl p-3 overflow-hidden">
            <div className="flex items-center gap-1.5 mb-2.5">
              <HugeiconsIcon icon={PackageIcon} size={14} strokeWidth={1.9} style={{ color: 'var(--accent-primary)' }} />
              <span className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: 'var(--text-secondary)' }}>{t('boxState')}</span>
            </div>
            <div className="grid grid-cols-3 gap-2 text-xs">
              <div className="flex justify-between gap-1">
                <span className="text-[10px] uppercase tracking-wide" style={{ color: 'var(--text-tertiary)' }}>{t('shortBox')}</span>
                <span className="font-semibold tabular-nums" style={{ color: 'var(--text-primary)' }}>
                  {formatPrice(riskInfo.short_box_lower)} - {formatPrice(riskInfo.short_box_upper)}
                </span>
              </div>
              <div className="flex justify-between gap-1">
                <span className="text-[10px] uppercase tracking-wide" style={{ color: 'var(--text-tertiary)' }}>{t('midBox')}</span>
                <span className="font-semibold tabular-nums" style={{ color: 'var(--text-primary)' }}>
                  {formatPrice(riskInfo.mid_box_lower)} - {formatPrice(riskInfo.mid_box_upper)}
                </span>
              </div>
              <div className="flex justify-between gap-1">
                <span className="text-[10px] uppercase tracking-wide" style={{ color: 'var(--text-tertiary)' }}>{t('longBox')}</span>
                <span className="font-semibold tabular-nums" style={{ color: 'var(--text-primary)' }}>
