...
export function IndicatorEditor({
  ...
  const t = (key: string) => {
    const translations: Record<string, Record<string, string>> = {
      ...
      timeframesDesc: { zh: '选择 K 线分析周期，星标为主周期（双击设置）', en: 'Select K-line timeframes, star marks primary (double-click)' },
      ...
      evaDataDesc: { zh: '专业加密货币量化数据服务', en: 'Professional crypto quant data service' },
      evaDataFeatures: { zh: 'AI500 · OI排行 · 资金流向 · 涨跌榜', en: 'AI500 · OI Ranking · Fund Flow · Price Ranking' },
    ...
    <div className="space-y-5">
        ...
        <div className="p-4">
            ...
            <div className="flex items-center gap-2">
              {hasApiKey ? (
                <span className="gl-badge gl-badge--buy">
                  <HugeiconsIcon icon={CheckmarkCircle01Icon} size={13} strokeWidth={1.9} />
                  {t('connected')}
                </span>
              ) : (
                <span className="gl-badge gl-badge--hold">
                  <HugeiconsIcon icon={AlertCircleIcon} size={13} strokeWidth={1.9} />
                  {t('notConfigured')}
                </span>
              )}
              <a
              ...
              >
                <HugeiconsIcon icon={LinkSquare02Icon} size={13} strokeWidth={1.9} />
                {t('viewApiDocs')}
              </a>
            </div>
          </div>
          ...
          <div>
            <label className="gl-field-label flex items-center gap-1.5">
              <HugeiconsIcon icon={BitcoinKeyIcon} size={13} strokeWidth={1.9} />
              {t('apiKey')}
            </label>
            <div className="flex items-center gap-2">
              <input
                ...
                onChange={(e) => !disabled && onChange({ ...config, eva_api_key: e.target.value })}
                disabled={disabled}
              ...
              {!disabled && !config.eva_api_key && (
                <button
                  type="button"
                  onClick={() => onChange({ ...config, eva_api_key: DEFAULT_EVA_API_KEY })}
          ...
          <div className="mt-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <HugeiconsIcon icon={ChartBarLineIcon} size={14} strokeWidth={1.9} style={{ color: config.enable_quant_data ? '#9fbcff' : 'var(--text-secondary)' }} />
                  ...
                  <div
                    ...
                    onClick={(e) => { e.stopPropagation(); !disabled && onChange({ ...config, enable_quant_data: !config.enable_quant_data }) }}
                  />
                </div>
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <HugeiconsIcon icon={ChartLineData01Icon} size={14} strokeWidth={1.9} style={{ color: config.enable_oi_ranking ? '#9fbcff' : 'var(--text-secondary)' }} />
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <HugeiconsIcon icon={ArrowDataTransferHorizontalIcon} size={14} strokeWidth={1.9} style={{ color: config.enable_netflow_ranking ? '#9fbcff' : 'var(--text-secondary)' }} />
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <HugeiconsIcon icon={TradeUpIcon} size={14} strokeWidth={1.9} style={{ color: config.enable_price_ranking ? '#9fbcff' : 'var(--text-secondary)' }} />
            ...
            {hasNofxosEnabled && !hasApiKey && (
              <div
                ...
                style={{
                  background: 'linear-gradient(180deg, rgba(246,70,93,0.12), rgba(246,70,93,0.04))',
                  border: '1px solid rgba(246,70,93,0.28)',
                  boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.05)',
                }}
              >
                <HugeiconsIcon icon={AlertCircleIcon} size={16} strokeWidth={1.9} className="flex-shrink-0" style={{ color: 'var(--binance-red)' }} />
                <span className="text-[10px] text-[var(--text-primary)]">
                  {language === 'zh' ? '请配置 API Key 以启用 EVA 数据源' : 'Please configure API Key to enable NofxOS data sources'}
                </span>
              </div>
            )}
          </div>
      ...
      <div className="gl-metal-panel rounded-2xl overflow-hidden">
        <div className="px-4 py-3 flex items-center gap-2 border-b border-[var(--panel-border)]">
          <div className="dash-ico shrink-0">
            <HugeiconsIcon icon={ChartBarLineIcon} size={16} strokeWidth={1.9} />
          </div>
          <span className="text-sm font-semibold gl-metal-shine">{t('marketData')}</span>
        ...
        <div className="p-3 space-y-4">
          ...
          <div className="flex items-center justify-between p-3 rounded-xl gl-onyx-panel overflow-hidden">
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="gl-badge gl-badge--info">
                    <HugeiconsIcon icon={SquareLock02Icon} size={11} strokeWidth={1.9} />
                    {t('required')}
                  </span>
                </div>
            ...
            <div className="gl-switch opacity-70 cursor-not-allowed" data-on="true" />
          </div>
                      ...
                      {categoryTfs.map((tf) => {
                        ...
                        return (
                          ...
                          >
                            {tf.label}
                            {isPrimary && (
                              <HugeiconsIcon icon={StarIcon} size={9} strokeWidth={2.2} style={{ color: '#9fbcff' }} />
                            )}
                          </button>
                        )
                      })}
      ...
      <div className="gl-metal-panel rounded-2xl overflow-hidden">
        <div className="px-4 py-3 flex items-center gap-2 border-b border-[var(--panel-border)]">
          ...
          <span className="text-sm font-semibold gl-metal-shine" style={{ animationDelay: '-1.2s' }}>{t('technicalIndicators')}</span>
        ...
        <div className="p-3">
          ...
          <div
            ...
            style={{
              background: 'linear-gradient(180deg, rgba(61,107,255,0.10), rgba(61,107,255,0.03))',
              border: '1px solid rgba(61,107,255,0.20)',
              boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.05)',
            }}
          >
            <HugeiconsIcon icon={InformationCircleIcon} size={14} strokeWidth={1.9} className="mt-0.5 flex-shrink-0" style={{ color: '#9fbcff' }} />
            ...
            {[
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center gap-2">
                    <HugeiconsIcon icon={Activity01Icon} size={13} strokeWidth={1.9} style={{ color: isOn ? '#9fbcff' : 'var(--text-secondary)' }} />
                    <span className="text-xs font-medium" style={{ color: 'var(--text-primary)' }}>{t(label)}</span>
                  </div>
                  <div
                    className="gl-switch"
                    data-on={isOn ? 'true' : 'false'}
                    onClick={() => !disabled && onChange({ ...config, [key]: !isOn })}
                  />
                </div>
      ...
      <div className="gl-metal-panel rounded-2xl overflow-hidden">
        <div className="px-4 py-3 flex items-center gap-2 border-b border-[var(--panel-border)]">
          ...
          <span className="text-sm font-semibold gl-metal-shine" style={{ animationDelay: '-2.4s' }}>{t('marketSentiment')}</span>
        ...
        <div className="p-3">
          <div className="grid grid-cols-3 gap-2">
            {[
              { key: 'enable_volume', label: 'volume', desc: 'volumeDesc', icon: ChartBarLineIcon },
              { key: 'enable_oi', label: 'oi', desc: 'oiDesc', icon: DatabaseIcon },
              { key: 'enable_funding_rate', label: 'fundingRate', desc: 'fundingRateDesc', icon: ArrowDataTransferHorizontalIcon },
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center gap-2">
                    <HugeiconsIcon icon={icon} size={13} strokeWidth={1.9} style={{ color: isOn ? '#9fbcff' : 'var(--text-secondary)' }} />
                    <span className="text-xs font-medium" style={{ color: 'var(--text-primary)' }}>{t(label)}</span>
                  </div>
                  <div
                    className="gl-switch"
