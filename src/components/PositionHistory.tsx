    ...
    <div className="gl-metal-panel rounded-xl p-4 relative overflow-hidden">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          {subtitle && (
            <div className="text-[11px] mt-2 font-medium" style={{ color: 'var(--text-tertiary)' }}>
              {subtitle}
            </div>
          )}
        </div>
  ...
  return (
    <div className="gl-onyx-panel rounded-2xl overflow-hidden p-4">
      <div className="flex items-center gap-2.5 mb-3.5">
        ...
        >
          <HugeiconsIcon icon={isLong ? TradeUpIcon : TradeDownIcon} size={17} strokeWidth={2} />
        </span>
        <span
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <div>
          ...
          <div className="font-semibold tabular-nums" style={{ color: avgPnl >= 0 ? 'var(--binance-green)' : 'var(--binance-red)' }}>
            {avgPnl >= 0 ? '+' : ''}
            {formatNumber(avgPnl)}
          </div>
        </div>
...
function PositionRow({ position }: { position: HistoricalPosition }) {
  ...
  return (
    ...
    >
      ...
      <td className="py-3 px-4 text-right tabular-nums text-xs" style={{ color: 'var(--text-secondary)' }}>
      ...
      {/* Duration */}
      <td className="py-3 px-4 text-center text-sm tabular-nums" style={{ color: 'var(--text-secondary)' }}>
        {formatDuration(holdingMinutes)}
      </td>

      {/* Exit Time */}
      <td className="py-3 px-4 text-right text-xs tabular-nums" style={{ color: 'var(--text-secondary)' }}>
...
export function PositionHistory({ traderId }: PositionHistoryProps) {
  ...
  if (loading) {
    return (
      <div className="gl-panel rounded-2xl overflow-hidden p-12 flex items-center justify-center gap-3" style={{ color: 'var(--text-secondary)' }}>
        <span className="animate-spin inline-flex" style={{ color: 'var(--accent-primary)' }}>
          <HugeiconsIcon icon={Loading03Icon} size={22} strokeWidth={2.2} />
        </span>
        <span className="text-sm font-medium">{t('positionHistory.loading', language)}</span>
      </div>
    )
  }
          ...
          <div className="flex items-center gap-3">
            ...
            {totalPages > 1 && (
              <div className="flex items-center gap-1.5">
                ...
                <span className="px-3 text-xs font-semibold tabular-nums" style={{ color: 'var(--text-primary)' }}>
