      ...
      <div className="gl-modal-overlay" onClick={onCancel}>
        ...
        >
          ...
          <div className="gl-modal-head">
            ...
            <button onClick={onCancel} className="gl-modal-close" aria-label={t('twoStageKey.cancelButton', language)}>
          ...
          <div className="gl-modal-scroll space-y-4">
            ...
            <div className="gl-seg">
              <div className="gl-seg-item" data-active={String(stage === 1)}>
                ...
                <span className="ml-1.5">{t('twoStageKey.stage1InputLabel', language)}</span>
              </div>
              <div className="gl-seg-item" data-active={String(stage === 2)}>
                <span className="tabular-nums">2</span>
                <span className="ml-1.5">{t('twoStageKey.stage2InputLabel', language)}</span>
              </div>
            </div>
              ...
              <>
                ...
                {error && (
                  <div className="gl-field-error flex items-center gap-1.5">
                    <HugeiconsIcon icon={InformationCircleIcon} size={14} strokeWidth={1.9} />
                    {error}
                  </div>
                )}
            ...
            {stage === 2 && clipboardStatus !== 'idle' && (
              <div
                ...
                style={{ boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.05), 0 0 18px rgba(61,107,255,0.14)' }}
              >
                {clipboardStatus === 'copied' && (
                  <div style={{ color: 'var(--text-secondary)' }}>
                    <div className="font-semibold text-sm flex items-center gap-2" style={{ color: 'var(--text-primary)' }}>
                      <HugeiconsIcon icon={CheckmarkCircle01Icon} size={16} strokeWidth={1.9} style={{ color: 'var(--binance-green)' }} />
                ...
                {clipboardStatus === 'failed' && manualObfuscationValue && (
                  <div style={{ color: 'var(--text-secondary)' }}>
                    <div className="font-semibold text-sm flex items-center gap-2" style={{ color: 'var(--text-primary)' }}>
                      <HugeiconsIcon icon={Key01Icon} size={16} strokeWidth={1.9} style={{ color: '#9fbcff' }} />
                      {t('twoStageKey.obfuscationManual', language)}
                    </div>
                    <div
                      className="text-xs mt-2 p-2.5 rounded-lg font-mono break-all tabular-nums"
                      style={{ background: 'var(--surface-primary)', border: '1px solid var(--panel-border)', color: 'var(--text-primary)' }}
                    >
