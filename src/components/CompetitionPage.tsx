...
export function CompetitionPage() {
          ...
          <div className="gl-panel rounded-2xl p-6 animate-pulse">
            <div className="flex items-center justify-between gap-4">
              <div className="space-y-3 flex-1">
                <div className="h-8 w-64 rounded" style={{ background: 'var(--surface-tertiary)' }}></div>
                <div className="h-4 w-48 rounded" style={{ background: 'var(--surface-tertiary)' }}></div>
              </div>
              <div className="h-12 w-32 rounded" style={{ background: 'var(--surface-tertiary)' }}></div>
            </div>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="gl-panel rounded-xl p-5 animate-pulse">
                <div className="h-3 w-20 mb-3 rounded" style={{ background: 'var(--surface-tertiary)' }}></div>
                <div className="h-8 w-28 rounded" style={{ background: 'var(--surface-tertiary)' }}></div>
              </div>
            ))}
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            <div className="gl-panel rounded-2xl p-6 animate-pulse">
              <div className="h-64 w-full rounded" style={{ background: 'var(--surface-tertiary)' }}></div>
            </div>
            <div className="gl-panel rounded-2xl p-6 animate-pulse space-y-3">
              <div className="h-20 w-full rounded" style={{ background: 'var(--surface-tertiary)' }}></div>
              <div className="h-20 w-full rounded" style={{ background: 'var(--surface-tertiary)' }}></div>
              <div className="h-20 w-full rounded" style={{ background: 'var(--surface-tertiary)' }}></div>
            </div>
          </div>
  ...
  const totalEquitySum = sortedTraders.reduce((acc, tr) => acc + (tr.total_equity ?? 0), 0)
  const avgPnlPct =
    sortedTraders.length > 0
      ? sortedTraders.reduce((acc, tr) => acc + (tr.total_pnl_pct ?? 0), 0) / sortedTraders.length
      : 0
    ...
    <DashPage>
      ...
      >
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2.5 flex-wrap">
                ...
                <span className="dash-chip">
                  <HugeiconsIcon icon={UserGroupIcon} size={13} strokeWidth={1.9} />
                  <span className="dash-chip-val tabular-nums">{competition.count}</span>
                  {t('traders', language)}
                </span>
          ...
          >
            <span className="dash-ico" style={{ color: 'var(--accent-primary)' }}>
              <HugeiconsIcon icon={CrownIcon} size={17} strokeWidth={1.9} />
            </span>
            <div className="text-left">
              <div className="text-[10px] font-semibold uppercase tracking-wider mb-0.5" style={{ color: 'var(--text-tertiary)' }}>
                {t('leader', language)}
              </div>
              ...
              <div
                ...
                style={{ color: leaderPnlPositive ? 'var(--binance-green)' : 'var(--binance-red)' }}
              >
                {leaderPnlPositive ? '+' : ''}
        ...
        >
          ...
          <div className="p-3 sm:p-4 space-y-2.5">
            {sortedTraders.map((trader, index) => {
              ...
              const rankIcon = index === 0 ? ChampionIcon : index === 1 ? Medal01Icon : index === 2 ? StarIcon : null
              ...
              return (
                <div
                  ...
                  style={{
                    background: isLeader ? 'var(--accent-primary-bg)' : 'var(--surface-primary)',
                    border: isLeader ? '1px solid var(--accent-primary-border)' : '1px solid var(--panel-border)',
                  }}
                >
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div
                        ...
                        style={
                          index < 3
                            ? { background: 'var(--accent-primary-bg)', color: 'var(--accent-primary)', border: '1px solid var(--accent-primary-border)' }
                            : { background: 'var(--surface-tertiary)', color: 'var(--text-secondary)' }
                        }
                      >
                        {rankIcon ? (
                          <HugeiconsIcon icon={rankIcon} size={15} strokeWidth={1.9} />
                        ) : (
                          index + 1
                        )}
                      </div>
                      {/* Punk Avatar */}
                      <div className="rounded-lg overflow-hidden shrink-0" style={{ border: '1px solid var(--panel-border)', lineHeight: 0 }}>
                        <PunkAvatar
                    ...
                    <div className="flex items-center gap-2 md:gap-3.5 flex-wrap md:flex-nowrap shrink-0">
                      ...
                      <div className="text-right min-w-[70px] md:min-w-[90px]">
                        ...
                        <div
                          ...
                          style={{ color: pnlPositive ? 'var(--binance-green)' : 'var(--binance-red)' }}
                        >
                          {pnlPositive ? '+' : ''}
                      ...
                      <div>
                        <span
                          ...
                          title={trader.is_running ? t('live', language) : t('liveBattle', language)}
                        >
      ...
      {competition.traders.length === 2 && (
        ...
        >
          <SectionHead icon={FlashIcon} title={t('headToHead', language)} delay="-3.6s" />
              ...
              return (
                ...
                >
                  <div className="text-center">
                    {/* Winner crown */}
                    {isWinning && (
                      <div className="flex justify-center mb-2" style={{ color: 'var(--binance-green)' }}>
                        <HugeiconsIcon icon={CrownIcon} size={18} strokeWidth={1.9} />
                      </div>
                    )}
                    {/* Avatar */}
                    <div className="flex justify-center mb-3">
                      <div className="rounded-xl overflow-hidden" style={{ border: '1px solid var(--panel-border)', lineHeight: 0 }}>
                        <PunkAvatar
                          seed={getTraderAvatar(trader.trader_id, trader.trader_name)}
                          size={56}
                          className="block"
                    ...
                    {hasValidData && isWinning && gap > 0 && (
                      ...
                      >
                        <HugeiconsIcon icon={Layers01Icon} size={13} strokeWidth={2} />
