...
import { useGLTF, Environment, Lightformer, AdaptiveDpr, AdaptiveEvents } from '@react-three/drei'
import { BallCollider, CuboidCollider, Physics, RigidBody } from '@react-three/rapier'
import {
  EffectComposer, Bloom, ChromaticAberration, Vignette, Noise,
  SMAA, ToneMapping, BrightnessContrast, HueSaturation,
  // extra "try-it" effects — all OFF in the fixed `ctrl` config below
  Glitch, Pixelation, DotScreen, Scanline, Sepia,
...
function SceneLighting() {
  return (
    <>
      ...
      <pointLight intensity={55} position={[0, 0, 5]} color="#ffffff" distance={30} decay={2} />
      <pointLight intensity={36} position={[0, 0, 15]} color="#ffffff" distance={28} decay={1.1} />
    </>
  )
}
...
function StudioEnv() {
  return (
    <Environment resolution={256} frames={1} background={false}>
      {/* big soft key */}
      <Lightformer intensity={2.0} form="rect" position={[0, 4, -6]} scale={[14, 7, 1]} color="#ffffff" />
      {/* vertical streaks the chrome catches */}
      <Lightformer intensity={3.6} form="rect" position={[-6, 1, -1]} rotation={[0, 0.35, 0]} scale={[1.4, 9, 1]} color="#ffffff" />
      <Lightformer intensity={3.2} form="rect" position={[6, -1, -1]} rotation={[0, -0.35, 0]} scale={[1.4, 9, 1]} color="#cfe0ff" />
      {/* cool rim + soft underfill */}
      <Lightformer intensity={1.4} form="ring" position={[0, -4, 4]} scale={[5, 5, 1]} color="#8fa8ff" />
      <Lightformer intensity={1.1} form="rect" position={[0, 0, 8]} scale={[10, 10, 1]} color="#9aa3c0" />
    </Environment>
  )
}
...
export default function CoinField({ collisionRef }: CoinFieldProps) {
  ...
  const mat = useMemo(
    () => ({ metalness: ctrl.metalness, roughness: ctrl.roughness, envMapIntensity: ctrl.envMapIntensity }),
    [ctrl.metalness, ctrl.roughness, ctrl.envMapIntensity]
  )
  const caOffset = useMemo(() => new THREE.Vector2(ctrl.caOffset, ctrl.caOffset), [ctrl.caOffset])
  ...
  useEffect(() => {
    ...
    if (!el) return
    const obs = new IntersectionObserver(([e]) => setPhysicsPaused(!e.isIntersecting), { threshold: 0.05 })
    obs.observe(el)
      ...
      <div ref={containerRef} style={{ width: '100%', height: '100%' }}>
        ...
        >
          ...
          <PhysicsResetContext.Provider value={resetContext}>
            <Physics timeStep={1 / 60} gravity={[0, 0, 0]} paused={physicsPaused} debug={DEBUG}>
              <Pointer />
              {boundariesActive && <Boundaries />}
              <ChatBoxCollider collisionRef={collisionRef} />
              <CoinsContainer connectors={connectors} visiblePositions={visiblePositions} mat={mat} />
            </Physics>
          ...
          <EffectComposer multisampling={0}>
            {[
              ...
              />,
              <ChromaticAberration key="ca" offset={caOffset} radialModulation modulationOffset={ctrl.caModulation} />,
              <HueSaturation key="hue" hue={0} saturation={ctrl.saturation} />,
              <BrightnessContrast key="bc" brightness={ctrl.brightness} contrast={ctrl.contrast} />,
              <Vignette key="vig" offset={ctrl.vignetteOffset} darkness={ctrl.vignetteDarkness} eskil={false} />,
              <Noise key="noise" premultiply blendFunction={BlendFunction.SOFT_LIGHT} opacity={ctrl.noiseOpacity} />,
              // experimental toggles (Leva → Effects) — only included when ON
              ctrl.pixelation && <Pixelation key="px" granularity={ctrl.pixelSize} />,
              ctrl.dotScreen && <DotScreen key="dot" scale={0.9} />,
              ctrl.scanline && <Scanline key="scan" density={ctrl.scanlineDensity} />,
              ctrl.sepia && <Sepia key="sepia" intensity={ctrl.sepiaIntensity} />,
  ...
  return (
    <>
      {connectors.map((coin, i) => (
        <CryptoCoins key={i} index={i} initialPosition={visiblePositions.current[i]} modelName={coin.name} mat={mat} />
      ))}
    </>
  )
}
...
function CryptoCoins({ initialPosition, modelName, mat }) {
  ...
  useFrame((_, delta) => {
    ...
    const force = coinSettings.attractionForce + distance * coinSettings.attractionDistanceMultiplier
    vec.set(-dx, -dy, -dz).normalize().multiplyScalar(force * distance * deltaMultiplier)
...
function Pointer() {
  ...
  useFrame(({ viewport }) => {
    ...
    currentPos.current.lerp(targetPos.current, idle ? 0.03 : pointerSettings.lerpSpeed)
...
function ChatBoxCollider({ collisionRef }: { collisionRef: React.RefObject<HTMLElement | null> }) {
  const body = useRef()
  ...
  useEffect(() => {
    ...
    if (!el) return
    const update = () => { rectRef.current = el.getBoundingClientRect() }
    update()
  ...
  return (
    <RigidBody ref={body} type="kinematicPosition" colliders={false} restitution={0.7} friction={0}>
