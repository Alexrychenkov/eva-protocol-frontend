...
export const BacktestResultsPanel = memo(function BacktestResultsPanel({
  ...
  const handleControl = async (action: 'pause' | 'resume' | 'stop') => {
    ...
    try {
      ...
      onToast({ text: tr('toasts.actionSuccess', { action, id: selectedRunId }), tone: 'success' })
      await refreshRuns()
    } catch (error: unknown) {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      onToast({ text: tr('toasts.exportSuccess', { id: selectedRunId }), tone: 'success' })
    } catch (error: unknown) {
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <StatCard
              ...
              value={(status?.equity ?? selectedRun?.summary?.equity_last ?? formBalance ?? 0).toFixed(2)}
              suffix="USDT"
              language={language}
            />
            <StatCard
            ...
            <div className="flex border-b" style={{ borderColor: 'var(--panel-border)' }}>
              {(['overview', 'chart', 'trades', 'decisions'] as ViewTab[]).map((tab) => (
                <button
                  ...
                  style={{
                    color: viewTab === tab ? 'var(--accent-primary)' : 'var(--text-secondary)',
                    borderBottom: viewTab === tab ? '2px solid var(--accent-primary)' : '2px solid transparent',
                  }}
                >
                  {tab === 'overview'
                    ? language === 'zh'
              ...
              {viewTab === 'overview' && (
                <div>
                  {equity && equity.length > 0 ? (
                    <BacktestChart key={selectedRunId} equity={equity} trades={trades ?? []} />
                  ) : (
              ...
              {viewTab === 'chart' && (
                <div className="space-y-6">
                  <div>
                    ...
                    {equity && equity.length > 0 ? (
                      <BacktestChart key={`${selectedRunId}-chart`} equity={equity} trades={trades ?? []} />
                    ) : (
                      <div className="py-12 text-center" style={{ color: 'var(--text-tertiary)' }}>
                        {tr('charts.equityEmpty')}
                      </div>
