...
export const SWR_OPTS = { revalidateOnFocus: false, dedupingInterval: 2000 } as const
...
export function findClosestEquityIndex(equity: BacktestEquityPoint[], tradeTs: number): number {
  ...
  return Math.abs(prev.ts - tradeTs) <= Math.abs(curr.ts - tradeTs) ? lo - 1 : lo
}
...
export function getStateIcon(state: string): ReactNode {
  switch (state) {
    case 'running':
      return createElement(HugeiconsIcon, { icon: Activity01Icon, ...stateIconProps })
    case 'completed':
      return createElement(HugeiconsIcon, { icon: CheckmarkCircle01Icon, ...stateIconProps })
    case 'failed':
    case 'liquidated':
      return createElement(HugeiconsIcon, { icon: CancelCircleIcon, ...stateIconProps })
    case 'paused':
      return createElement(HugeiconsIcon, { icon: PauseIcon, ...stateIconProps })
    default:
      return createElement(HugeiconsIcon, { icon: Clock01Icon, ...stateIconProps })
  }
}
