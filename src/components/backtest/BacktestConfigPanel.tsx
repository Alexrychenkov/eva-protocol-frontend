...
import type { AIModel, BacktestRunMetadata, BacktestRunsResponse, Strategy } from '../../types'
import {
  type WizardStep,
  type ToastPayload,
  TIMEFRAME_OPTIONS,
...
export const BacktestConfigPanel = memo(function BacktestConfigPanel({
  ...
  const { data: aiModels } = useSWR<AIModel[]>('ai-models', api.getModelConfigs, { ...SWR_OPTS, refreshInterval: 60000 })
  const { data: strategies } = useSWR<Strategy[]>('strategies', api.getStrategies, { ...SWR_OPTS, refreshInterval: 60000 })
  ...
  const handleStart = async (event: FormEvent) => {
    ...
    try {
      ...
      onToast({ text: tr('toasts.startSuccess', { id: payload.run_id }), tone: 'success' })
  ...
  const handleRerun = async (runId: string) => {
    try {
      ...
      setWizardStep(1)
      onToast({ text: language === 'zh' ? '已加载配置，缓存已启用' : 'Config loaded, cache enabled for re-run', tone: 'success' })
    } catch (err) {
      onToast({ text: err instanceof Error ? err.message : (language === 'zh' ? '加载配置失败' : 'Failed to load config'), tone: 'error' })
    }
  }
