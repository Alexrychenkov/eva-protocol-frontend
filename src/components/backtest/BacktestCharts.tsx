...
import { downsampleSeries, findClosestEquityIndex, MAX_CHART_POINTS } from './backtestShared'
...
export const ProgressRing = memo(function ProgressRing({ progress, size = 120 }: { progress: number; size?: number }) {
  const strokeWidth = 8
...
export const CandlestickChartComponent = memo(function CandlestickChartComponent({
  runId,
  trades,
  language,
}: {
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((data: BacktestKlinesResponse) => {
        ...
        createSeriesMarkers(candleSeries, buildTradeMarkers(data.klines, symbolTrades))
...
export const TradeTimeline = memo(function TradeTimeline({ trades }: { trades: BacktestTradeEvent[] }) {
