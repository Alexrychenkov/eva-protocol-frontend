    ...
    <Select.Root value={value} onValueChange={onChange}>
      <Select.Trigger className="gl-select-trigger" aria-label={ariaLabel}>
        <span className="gl-select-trigger-inner">
          {selected && <TokenLogo image={selected.image} label={selected.label} />}
          <span className="gl-select-trigger-label">{selected?.label ?? ''}</span>
        </span>
        <Select.Icon asChild>
      ...
      <Select.Portal>
        <Select.Content className="gl-select-content" position="popper" sideOffset={6}>
          <Select.Viewport className="gl-select-viewport">
            {options.map((o) => (
              <Select.Item key={o.value} value={o.value} className="gl-select-item">
