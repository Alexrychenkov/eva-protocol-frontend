...
export function SettingsModal({ isOpen, onClose, language }: SettingsModalProps) {
  ...
  const loadSecurity = useCallback(async () => {
    ...
    if (result.success) {
      ...
      setError(result.message || (isEn ? 'Failed to load settings' : '加载设置失败'))
    }
    setLoading(false)
  ...
  const handleStartSetup = async () => {
    ...
    if (result.success && result.otpSecret) {
      ...
      setError(result.message || (isEn ? 'Failed to start 2FA setup' : '无法开始两步验证设置'))
    }
    setSubmitting(false)
  }
  ...
  const handleEnable2FA = async () => {
    ...
    setSubmitting(true)
    setError('')
    const result = await completeRegistration(user.id, otpCode, { redirect: false })
    if (result.success) {
      setOtpEnabled(true)
      resetFlow()
      toast.success(isEn ? 'Two-factor authentication enabled' : '两步验证已启用')
      await loadSecurity()
    } else {
      setError(result.message || (isEn ? 'Invalid verification code' : '验证码错误'))
    }
    setSubmitting(false)
  }
  ...
  const copySecret = () => {
    if (!otpSecret) return
    copyWithToast(otpSecret, t('copiedToClipboard', language), t('copyFailed', language))
    setCopied(true)
      ...
      >
        <div className="gl-modal-head">
          <div className="flex items-center gap-3 min-w-0">
            <span className="dash-ico shrink-0">
              <HugeiconsIcon icon={Settings02Icon} size={18} strokeWidth={1.9} />
            </span>
          ...
          <button onClick={onClose} className="gl-modal-close" aria-label={isEn ? 'Close' : '关闭'}>
        ...
        <div className="gl-modal-scroll space-y-5">
          <section className="gl-onyx-panel rounded-xl p-4">
            <p className="text-[10px] font-mono tracking-[0.18em] mb-2" style={{ color: 'var(--text-tertiary)' }}>
              {isEn ? 'ACCOUNT' : '账户'}
            </p>
          ...
          <section className="gl-onyx-panel rounded-xl p-4 space-y-4">
            <div className="flex items-start justify-between gap-4">
              ...
              {otpEnabled ? (
                <span className="dash-chip shrink-0" style={{ color: 'var(--binance-green)' }}>
            ...
            {loading ? (
              ...
              <div className="space-y-4">
                {(step === 'setup' || step === 'verify') && user?.email && (
                  ...
                  >
                    <p className="text-[10px] font-mono tracking-wider mb-4" style={{ color: 'rgba(255,255,255,0.4)' }}>
                ...
                {step === 'setup' ? (
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex gap-3">
                      ...
                      >
                        {submitting ? (isEn ? 'Verifying…' : '验证中…') : (isEn ? 'Enable 2FA' : '确认启用')}
                      </button>
                    </div>
                  </div>
                )}
            ...
            {error && (
              <div className="gl-field-error">{error}</div>
            )}
          </section>
        </div>

        <div className="gl-modal-foot">
          <button type="button" onClick={onClose} className="gl-modal-btn-ghost inline-flex items-center justify-center px-5 py-2.5 rounded-xl text-sm font-semibold">
            {isEn ? 'Close' : '关闭'}
          </button>
        </div>
      </div>
