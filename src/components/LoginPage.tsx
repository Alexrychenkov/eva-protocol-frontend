  ...
  return (
    <div className="min-h-screen relative overflow-hidden" style={{ background: '#04050A' }}>
      {/* Strands WebGL background (ReactBits, OGL) — recolored to EVA dark-blue /
          white / purple, wider & longer (low taper). The container is offset by
          the 500px poster panel on desktop so the strands are CENTERED on the
          form, not the viewport. Full-width on mobile (panel hidden). */}
        ...
        <div className="min-h-full flex items-center justify-center px-6 py-12">
          ...
          >
            ...
            <div
              ...
              style={{ background: 'linear-gradient(90deg, transparent 0%, rgba(61,107,255,0.9) 35%, rgba(61,107,255,0.5) 65%, transparent 100%)' }}
            />
            ...
            <div className="p-7 sm:p-9">
              ...
              <div className="mb-8">
                <p className="text-[10px] font-mono tracking-[0.25em] mb-2.5" style={{ color: '#3d6bff', opacity: 0.8 }}>
                  EVA / {step === 'login' ? 'SIGN IN' : step === 'otp' ? 'VERIFY' : 'SECURE SETUP'}
                </p>
                <h1 className="text-[1.6rem] font-bold mb-1 tracking-tight gl-metal-shine inline-block">
                  {step === 'login'
                    ? (language === 'zh' ? '登录' : 'Sign in')
                    : step === 'otp'
                      ? (language === 'zh' ? '验证码' : 'Verification')
                      : (language === 'zh' ? '设置两步验证' : 'Set up 2FA')}
                </h1>
                <p className="text-[13px]" style={{ color: 'rgba(255,255,255,0.38)' }}>
                  {step === 'login'
                    ? (language === 'zh' ? '使用您的账户登录' : 'Access your trading dashboard')
                    : step === 'otp'
                      ? (language === 'zh' ? '输入您的验证码' : 'Enter your 6-digit code')
                      : (language === 'zh' ? '扫描二维码完成设置' : 'Secure your account with 2FA')}
                </p>
              </div>
              ...
              {adminMode ? (
                <form onSubmit={handleAdminLogin} className="space-y-5">
                  <div>
                    <label className="block text-xs font-mono tracking-wider mb-2" style={{ color: 'rgba(255,255,255,0.45)' }}>
                      ADMIN KEY
                    </label>
                    <input
                      type="password"
                  ...
                  <SubmitBtn loading={loading} label={language === 'zh' ? '验证中...' : 'Verifying...'} idleLabel={language === 'zh' ? '登录' : 'Sign in'} />
                </form>
                ...
                <div className="space-y-6">
                  <div className="rounded-xl p-5 text-center" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
                    ...
                    <div className="flex items-center gap-2 justify-center">
                      <code className="text-[11px] px-3 py-2 rounded-lg font-mono" style={{ background: 'rgba(61,107,255,0.08)', border: '1px solid rgba(61,107,255,0.2)', color: '#3d6bff' }}>
                        {otpSecret}
                      </code>
                      <button
                        type="button"
                  ...
                  <div className="space-y-4">
                    {[
                      ...
                      { n: '02', title: language === 'zh' ? '扫描并验证' : 'Scan & verify', sub: language === 'zh' ? '扫描上方二维码，然后输入6位验证码' : 'Scan the QR code, then enter the 6-digit code' },
                    ].map((item) => (
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-2">
                        <label className="text-[11px] font-mono tracking-wider" style={{ color: 'rgba(255,255,255,0.4)' }}>
                          {language === 'zh' ? '密码' : 'PASSWORD'}
                        </label>
                        <a href="/reset-password" className="text-[11px] font-mono transition-opacity hover:opacity-70" style={{ color: '#3d6bff' }}>
            ...
            <div className="mt-7 text-center space-y-3">
              {registrationEnabled && (
                <p className="text-[13px]" style={{ color: 'rgba(255,255,255,0.62)' }}>
                  ...
                  <a href="/register" className="font-medium transition-opacity hover:opacity-70" style={{ color: '#3d6bff' }}>
...
function SubmitBtn({ loading, label, idleLabel }: { loading: boolean; label: string; idleLabel: string }) {
  return (
    <button
      type="submit"
      disabled={loading}
