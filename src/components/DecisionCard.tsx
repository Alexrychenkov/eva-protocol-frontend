...
const ACTION_CONFIG: Record<string, { color: string; badge: string; Icon: typeof Robot01Icon; label: string }> = {
  open_long: { color: '#0ECB81', badge: 'gl-badge--buy', Icon: ArrowUpRight01Icon, label: 'LONG' },
  open_short: { color: '#F6465D', badge: 'gl-badge--sell', Icon: ArrowDownRight01Icon, label: 'SHORT' },
  close_long: { color: 'var(--accent-primary)', badge: 'gl-badge--info', Icon: Cancel01Icon, label: 'CLOSE' },
  close_short: { color: 'var(--accent-primary)', badge: 'gl-badge--info', Icon: Cancel01Icon, label: 'CLOSE' },
  hold: { color: '#848E9C', badge: 'gl-badge--hold', Icon: PauseIcon, label: 'HOLD' },
  wait: { color: '#848E9C', badge: 'gl-badge--hold', Icon: Clock01Icon, label: 'WAIT' },
}
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <div
            ...
            style={{
              background: action.success ? 'var(--binance-green)' : 'var(--binance-red)',
      ...
      {isOpen && (
        <div className="grid grid-cols-4 gap-3 mt-3 pt-3" style={{ borderTop: '1px solid var(--panel-border)' }}>
          ...
          {/* Stop Loss */}
          <div className="text-center">
            <div className="text-[10px] uppercase tracking-wider mb-1" style={{ color: 'var(--binance-red)' }}>
              {t('stopLoss', language)}
            </div>
            <div className="font-mono font-semibold tabular-nums" style={{ color: 'var(--binance-red)' }}>
          ...
          {/* Take Profit */}
          <div className="text-center">
            <div className="text-[10px] uppercase tracking-wider mb-1" style={{ color: 'var(--binance-green)' }}>
              {t('takeProfit', language)}
            </div>
            <div className="font-mono font-semibold tabular-nums" style={{ color: 'var(--binance-green)' }}>
          ...
          <div className="text-center">
            ...
            <div className="font-mono font-semibold tabular-nums" style={{ color: 'var(--nofx-gold)' }}>
              {action.leverage}x
            </div>
          </div>
        </div>
      ...
      {isOpen && action.stop_loss && action.take_profit && action.price && (
        <div className="mt-3 pt-3 flex items-center justify-between" style={{ borderTop: '1px solid var(--panel-border)' }}>
          <span className="text-xs flex items-center gap-1.5 uppercase tracking-wider" style={{ color: 'var(--text-tertiary)' }}>
            <HugeiconsIcon icon={BalanceScaleIcon} size={14} strokeWidth={1.9} />
      ...
      {action.reasoning && (
        <div className="mt-3 pt-3" style={{ borderTop: '1px solid var(--panel-border)' }}>
          <div className="text-xs line-clamp-2 flex items-start gap-2" style={{ color: 'var(--text-secondary)' }}>
            <HugeiconsIcon icon={Idea01Icon} size={14} strokeWidth={1.9} className="shrink-0 mt-0.5" style={{ color: 'var(--accent-primary)' }} />
            {action.reasoning}
          </div>
        </div>
      )}
      ...
      {action.error && (
        ...
        >
          <span className="flex items-center gap-2">
            <HugeiconsIcon icon={CancelCircleIcon} size={14} strokeWidth={1.9} className="shrink-0" />
            {action.error}
          </span>
        </div>
      )}
  ...
  return (
    <div className="gl-metal-panel rounded-2xl p-5 overflow-hidden transition-all duration-300 hover:translate-y-[-2px]">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className={`gl-badge ${decision.success ? 'gl-badge--buy' : 'gl-badge--sell'}`}>
          <HugeiconsIcon icon={decision.success ? CheckmarkCircle01Icon : CancelCircleIcon} size={13} strokeWidth={2} />
      ...
      <div className="space-y-2">
        ...
        {decision.cot_trace && (
          <div>
            ...
            >
              <div className="flex items-center gap-2">
                <HugeiconsIcon icon={AiBrain01Icon} size={16} strokeWidth={1.9} className="shrink-0" style={{ color: 'var(--nofx-gold)' }} />
      ...
      {decision.execution_log && decision.execution_log.length > 0 && (
        ...
        >
          {decision.execution_log.map((log, index) => (
            <div key={`${log}-${index}`} style={{ color: 'var(--text-secondary)' }}>
              {log}
            </div>
          ))}
        </div>
      ...
      {decision.error_message && (
        ...
        >
          <HugeiconsIcon icon={CancelCircleIcon} size={16} strokeWidth={1.9} className="shrink-0" />
          {decision.error_message}
        </div>
      )}
    </div>
