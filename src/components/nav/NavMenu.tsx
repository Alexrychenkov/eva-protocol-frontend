...
import { NAV, CHEVRON_ICON, type NavGroup, type NavLeaf, type NavPage } from './navConfig'
...
function GroupMenu({ group, isEn, currentPage, onLeaf }: NavCommon & { group: NavGroup }) {
  ...
  const active = currentPage !== undefined && group.items.some((i) => i.page === currentPage)

  return (
    <div className="gl-nav-group" onMouseEnter={openNow} onMouseLeave={closeSoon}>
      <button
        type="button"
        ...
        onClick={() => setOpen((o) => !o)}
      >
        <HugeiconsIcon icon={group.icon} size={17} strokeWidth={1.8} className="gl-nav-ico" />
        <span>{isEn ? group.label : group.zh}</span>
        <HugeiconsIcon icon={CHEVRON_ICON} size={13} strokeWidth={2.2} className="gl-nav-chev" />
      </button>
        ...
        >
          {group.items.map((leaf) => (
            <button
              ...
              onClick={() => { setOpen(false); onLeaf(leaf) }}
            >
              <span className="gl-nav-dd-ico">
                <HugeiconsIcon icon={leaf.icon} size={18} strokeWidth={1.8} />
              </span>
              <span className="gl-nav-dd-text">
                <span className="gl-nav-dd-label">{isEn ? leaf.label : leaf.zh}</span>
                <span className="gl-nav-dd-desc">{isEn ? leaf.desc : leaf.descZh}</span>
              </span>
            </button>
          ))}
        </div>
...
function LinkItem({ leaf, isEn, currentPage, onLeaf }: NavCommon & { leaf: NavLeaf }) {
  ...
  return (
    <button
      ...
      onClick={() => onLeaf(leaf)}
    >
      <HugeiconsIcon icon={leaf.icon} size={17} strokeWidth={1.8} className="gl-nav-ico" />
...
function MobileRow({ leaf, isEn, currentPage, onLeaf }: NavCommon & { leaf: NavLeaf }) {
  const active = leaf.page === currentPage
  return (
    <button type="button" className={`gl-mnav-row${active ? ' is-active' : ''}`} onClick={() => onLeaf(leaf)}>
      ...
      <HugeiconsIcon icon={CHEVRON_ICON} size={16} strokeWidth={2} className="gl-mnav-arrow" />
    </button>
  )
}
...
export function MobileNav(props: NavCommon) {
  ...
  return (
    <div className="flex flex-col gap-5">
      {NAV.map((entry) => {
        if (entry.kind === 'link') {
          return <MobileRow key={entry.leaf.page} leaf={entry.leaf} {...props} />
        }
        const g = entry.group
        return (
