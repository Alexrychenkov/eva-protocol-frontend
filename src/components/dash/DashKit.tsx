...
export function EmptyState({
    ...
    return (
        <div className={`flex items-center justify-center relative z-10 px-6 ${compact ? 'py-14' : 'min-h-[60vh]'}`}>
            <div className="text-center max-w-md mx-auto">
                <div
                    className="mx-auto mb-6 rounded-2xl flex items-center justify-center dash-kpi-ico"
                    style={{ width: compact ? 56 : 80, height: compact ? 56 : 80, borderRadius: compact ? 16 : 20 }}
                >
                    <HugeiconsIcon icon={icon} size={compact ? 26 : 34} strokeWidth={1.6} />
                </div>
                <h2 className={`font-bold mb-3 gl-metal-text ${compact ? 'text-lg' : 'text-2xl'}`}>{title}</h2>
                {description && (
                    <p className={`mb-7 ${compact ? 'text-sm' : 'text-base'}`} style={{ color: 'var(--text-secondary)' }}>
                        {description}
                    </p>
                )}
                {action && (
                    <button onClick={action.onClick} className="gl-navbar-btn px-6 py-3 rounded-xl text-sm font-semibold">
                        {action.label}
                    </button>
                )}
            </div>
            ...
            <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                    ...
                    {change !== undefined && (
                        ...
                        >
                            <HugeiconsIcon icon={positive ? ArrowUpRight01Icon : ArrowDownRight01Icon} size={14} strokeWidth={2.4} />
                            <span className="tabular-nums">{positive ? '+' : ''}{change.toFixed(2)}%</span>
                        </div>
                    )}
                    {subtitle && (
                        <div className="mt-2 text-[11px] font-medium" style={{ color: 'var(--text-tertiary)' }}>
                            {subtitle}
                        </div>
                    )}
                </div>
...
export function SectionHead({
    ...
    return (
        <div className="flex items-center justify-between gap-3 px-4 sm:px-5 py-3.5 border-b" style={{ borderColor: 'var(--panel-border)' }}>
            <div className="flex items-center gap-2.5 min-w-0">
                <span className="dash-ico">
                    <HugeiconsIcon icon={icon} size={16} strokeWidth={1.9} />
                </span>
                <h2 className="text-sm font-semibold uppercase tracking-wider gl-metal-shine truncate" style={{ animationDelay: delay }}>
                    {title}
                </h2>
            </div>
            {right}
