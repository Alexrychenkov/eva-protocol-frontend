...
import { Shield, ShieldCheck, Sparkles, ArrowRight, Settings } from 'lucide-react'
      ...
      >
        <div className="gl-modal-head">
          <div className="flex items-center gap-3 min-w-0 flex-1">
            <span className="dash-ico shrink-0">
              <Sparkles className="w-[18px] h-[18px]" style={{ color: '#bcd0ff' }} />
            </span>
            <div className="min-w-0">
              <p className="text-[10px] font-mono tracking-[0.2em] mb-0.5" style={{ color: 'var(--text-tertiary)' }}>
          ...
          <div className="flex items-center gap-2">
            {STEPS.map((_, i) => (
              <div
                ...
                style={{
                  background: i <= stepIndex ? '#3d6bff' : 'rgba(255,255,255,0.08)',
                  boxShadow: i === stepIndex ? '0 0 12px rgba(61,107,255,0.45)' : 'none',
                }}
              />
            ))}
          </div>
          <p className="text-[11px] font-mono mt-2" style={{ color: 'var(--text-tertiary)' }}>
            {isEn ? `Step ${stepIndex + 1} of ${STEPS.length}` : `第 ${stepIndex + 1} / ${STEPS.length} 步`}
          </p>
        </div>
            ...
            <div className="gl-onyx-panel rounded-xl p-5 space-y-4">
              <div className="flex flex-col items-center text-center py-2">
                <div
                  ...
                  style={{
                    background: otpEnabled ? 'rgba(14, 203, 129, 0.12)' : 'rgba(61,107,255,0.12)',
                    border: otpEnabled
                ...
                >
                  {otpEnabled ? (
                    <ShieldCheck className="w-7 h-7" style={{ color: 'var(--binance-green)' }} />
                  ) : (
                ...
                <h3 className="text-base font-semibold mb-2" style={{ color: 'var(--text-primary)' }}>
                  {otpEnabled
                    ? (isEn ? 'Your account is protected' : '账户已受保护')
                    : (isEn ? 'Secure your account' : '保护您的账户')}
                </h3>
                <p className="text-sm leading-relaxed max-w-sm" style={{ color: 'var(--text-secondary)' }}>
                  {loadingSecurity
                    ? (isEn ? 'Checking security settings…' : '正在检查安全设置…')
                    : otpEnabled
                      ? (isEn
              ...
              {!otpEnabled && !loadingSecurity && (
                ...
                >
                  ...
                  {isEn ? 'Open settings to enable 2FA' : '打开设置启用两步验证'}
                </button>
              )}
            </div>
          )}
          ...
          {step === 'welcome' && (
            <div className="gl-onyx-panel rounded-xl p-5 space-y-4">
              ...
              <ul className="space-y-2.5 text-sm" style={{ color: 'var(--text-secondary)' }}>
                {[
                  isEn ? 'Configure AI models and exchanges' : '配置 AI 模型与交易所',
                  isEn ? 'Create and launch AI traders' : '创建并启动 AI 交易员',
                  isEn ? 'Monitor performance on the dashboard' : '在仪表盘查看表现',
                ].map((item) => (
                  <li key={item} className="flex items-start gap-2.5">
                    <span
                      className="mt-1.5 w-1.5 h-1.5 rounded-full shrink-0"
                      style={{ background: '#3d6bff', boxShadow: '0 0 6px rgba(61,107,255,0.8)' }}
                    />
                    <span>{item}</span>
                  </li>
                ))}
        ...
        <div className="gl-modal-foot justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {step === 'welcome'
                ? (isEn ? 'Go to AI Traders' : '前往 AI 交易员')
                : otpEnabled
                  ? (isEn ? 'Continue' : '继续')
                  : (isEn ? 'Continue without 2FA' : '暂不启用，继续')}
