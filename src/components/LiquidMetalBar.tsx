...
export default function LiquidMetalBar({ children, type = 'button', onClick, disabled = false, className = '' }: Props) {
  ...
  useEffect(() => {
    ...
    try {
      mount = new ShaderMount(el, liquidMetalFragmentShader, UNIFORMS, undefined, SPEED)
    } catch {
      /* WebGL unavailable → static metal `.lm-bar` base remains */
    }
    return () => {
      try { mount?.dispose() } catch { /* context already gone */ }
    }
  }, [])

  return (
    <button type={type} onClick={onClick} disabled={disabled} className={`lm-bar ${className}`}>
