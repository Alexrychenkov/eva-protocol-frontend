...
async function fetchSolanaTokenBalance(ownerAddress: string, mintAddress: string): Promise<number> {
  const isSolanaAddr = !ownerAddress.startsWith('0x') && ownerAddress.length >= 32
  if (!isSolanaAddr) {
  ...
  useEffect(() => {
    ...
    if (!isSolanaMintConfigured()) {
      if (previewMode === 'eligible' || previewMode === 'ineligible') {
        const totalBalance = previewMode === 'eligible' ? UPGRADE_MIN_EVA_BALANCE : UPGRADE_MIN_EVA_BALANCE / 3
        setState({
    ...
    ;(async () => {
      try {
        const totalBalance = await fetchSolanaTokenBalance(address, EVA_SOLANA_MINT)
        ...
        if (cancelled) return
        const message = error instanceof Error ? error.message : 'Failed to read EVA Solana balances'
